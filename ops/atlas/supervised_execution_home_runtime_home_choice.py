...
from ops.atlas.supervised_execution_home_runtime_home_selection import (
    CONTRACT_RECEIPT_REFS as RUNTIME_HOME_SELECTION_CONTRACT_RECEIPT_REFS,
    QUESTION_PROMPT as RUNTIME_HOME_SELECTION_QUESTION_PROMPT,
    RUNTIME_HOME_SELECTION_STATUS_ADMISSIBLE,
)

RUNTIME_HOME_CHOICE_STATUS_ADMISSIBLE = "runtime_home_choice_admissible"
NO_RUNTIME_HOME_CHOICE = "no_runtime_home_choice"
QUESTION_PROMPT = (
    "Which later admitted surface, if any, may choose one runtime home for the already-admitted "
    "runtime-home-selection seam for `stack supervised-execution-home` without choosing one "
    "runtime home now or widening into concrete implementation-surface choice, _stack "
    "implementation, worker authority, owner-repo edits, actual owner-side mutation authority, "
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == RUNTIME_HOME_SELECTION_QUESTION_PROMPT
        ...
        == list(RUNTIME_HOME_SELECTION_CONTRACT_RECEIPT_REFS)
    )
...
def _attempt_reasons(bundle: Mapping[str, Any]) -> list[str]:
    ...
    checks = (
        ...
        (RUNTIME_HOME_KEYS, "runtime_home_choice_attempted"),
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("runtime_home_selection_status"))
        != RUNTIME_HOME_SELECTION_STATUS_ADMISSIBLE
    ):
        return ["runtime_home_selection_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(bundle.get("runtime_home_selection_question"), candidate_path):
        reasons.append("runtime_home_selection_question_not_explicit")
    if _normalized_string_list(bundle.get("runtime_home_selection_reasons")):
        reasons.append("runtime_home_selection_reasons_present")
...
def evaluate_supervised_execution_home_runtime_home_choice(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_home_choice_status": (
            RUNTIME_HOME_CHOICE_STATUS_ADMISSIBLE if not reasons else NO_RUNTIME_HOME_CHOICE
        ),
        "runtime_home_choice_question": _question_card(candidate_path) if not reasons else None,
        "runtime_home_choice_reasons": reasons,
    }
