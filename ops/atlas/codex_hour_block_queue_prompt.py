...
def build_report_from_scheduler_report(scheduler_report: dict[str, Any]) -> OrderedDict[str, Any]:
    ...
    report = OrderedDict(
        [
            ...
            ("planner", OrderedDict([("status", scheduler_report.get("status")), ("selected_marker", selected_marker), ("selected_packet", selected_packet), ("selected_jobs", scheduler_report.get("selected_jobs", [])), ("candidate_count", scheduler_report.get("candidate_count")), ("held_count", len(scheduler_report.get("skipped_candidates", []))), ("safe_candidate_count", len(scheduler_report.get("candidates", []))), ("safe_candidates", scheduler_report.get("candidates", []))])),
            ("suppression", OrderedDict([("status", scheduler_report.get("status")), ("decision", scheduler_report.get("decision")), ("root_clean", True), ("validation_state", scheduler_report.get("validation_state", {})), ("owner_lane_fallback_forbidden", False), ("owner_lane_requires_admitted_standing_packet", True), ("exact_packet_available", selected_packet is not None), ("safe_to_continue", should_generate_queue)])),
            ...
            ("excluded_surfaces", scheduler_report.get("scope_lock", {}).get("forbidden_writer_scopes", [])),
