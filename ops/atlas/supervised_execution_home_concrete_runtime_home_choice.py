...
CONCRETE_RUNTIME_HOME_CHOICE_STATUS_ADMISSIBLE = (
    "concrete_runtime_home_choice_admissible"
)
NO_CONCRETE_RUNTIME_HOME_CHOICE = "no_concrete_runtime_home_choice"
QUESTION_PROMPT = (
    "Which later admitted surface, if any, may choose one concrete runtime home for the "
    "already-admitted runtime-home-choice seam for `stack supervised-execution-home` "
    "without choosing one concrete runtime home now or widening into concrete "
    "implementation-surface choice, _stack implementation, worker authority, owner-repo "
    "edits, actual owner-side mutation authority, Playbook doctrine export, or "
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == RUNTIME_HOME_CHOICE_QUESTION_PROMPT
        ...
        == list(RUNTIME_HOME_CHOICE_CONTRACT_RECEIPT_REFS)
    )
...
def _attempt_reasons(bundle: Mapping[str, Any]) -> list[str]:
    ...
    checks = (
        ...
        (RUNTIME_HOME_KEYS, "concrete_runtime_home_choice_attempted"),
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("runtime_home_choice_status"))
        != RUNTIME_HOME_CHOICE_STATUS_ADMISSIBLE
    ):
        return ["runtime_home_choice_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(bundle.get("runtime_home_choice_question"), candidate_path):
        reasons.append("runtime_home_choice_question_not_explicit")
    if _normalized_string_list(bundle.get("runtime_home_choice_reasons")):
        reasons.append("runtime_home_choice_reasons_present")
...
def evaluate_supervised_execution_home_concrete_runtime_home_choice(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
