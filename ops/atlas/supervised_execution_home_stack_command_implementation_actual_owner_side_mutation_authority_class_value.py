...
from ops.atlas.supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_authority_class_choice import (
    ...
    CONTRACT_RECEIPT_REFS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE_CONTRACT_RECEIPT_REFS,
    LIVE_REPO_OR_TRANSCRIPT_KEYS,
    PASSTHROUGH_FIELDS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE_PASSTHROUGH_FIELDS,
    PLAYBOOK_DOCTRINE_EXPORT_KEYS,
    PROTECTED_SURFACE_EXCEPTION_KEYS,
    QUESTION_PROMPT as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE_QUESTION_PROMPT,
    STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE_STATUS_ADMISSIBLE,
    STACK_COMMAND_IMPLEMENTATION_KEYS,
    STACK_EXECUTION_HOME_KEYS,
)

STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_VALUE_STATUS_ADMISSIBLE = (
    "stack_command_implementation_actual_owner_side_mutation_authority_class_value_admissible"
)
NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_VALUE = (
    "no_stack_command_implementation_actual_owner_side_mutation_authority_class_value"
)
QUESTION_PROMPT = (
    "Which actual owner-side mutation authority-class value, if any, may any "
    "later admitted surface choose for the already-admitted _stack command "
    "implementation actual owner-side mutation surface seam, the now-bounded "
    "actual owner-side mutation authority-class seam, the now-bounded actual "
    "owner-side mutation surface-realization seam, the now-bounded actual "
    "owner-side mutation authority-class-realization seam, the now-bounded "
    "actual owner-side mutation surface-choice seam, and the now-bounded "
    "actual owner-side mutation authority-class-choice seam for `stack "
    "supervised-execution-home` without choosing it now, one actual owner-side "
    "mutation authority class now, one actual owner-side mutation surface now, "
    "one actual `_stack` execution-home surface, one actual concrete command "
    "file, one actual downstream runtime-home value placement, one actual "
    "concrete `_stack` command implementation-surface choice, one actual "
    "`_stack` command implementation, one Playbook doctrine export path, "
    "deploy or publication work, or one protected-surface touch?"
)
CONTRACT_RECEIPT_REFS = (
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-AUTHORITY-CLASS-VALUE-CONTRACT-FREEZE-PASS-757-2026-06-26.md",
    ...
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-AUTHORITY-CLASS-VALUE-FIRST-IMPLEMENTATION-ADMISSION-PASS-760-2026-06-26.md",
)
...
ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_VALUE_KEYS = {
    "actual_owner_side_mutation_authority_class_value",
    "actual_owner_side_mutation_authority_class_value_candidate",
    "actual_owner_side_mutation_authority_class_value_choice",
    "actual_owner_side_mutation_authority_class_value_selection",
    "actual_owner_side_mutation_authority_value",
    "actual_owner_side_mutation_authority_value_choice",
    "mutation_execution_authority_value",
    "owner_repo_write_authority_value",
}
PASSTHROUGH_FIELDS = (
    *STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE_PASSTHROUGH_FIELDS,
    "stack_command_implementation_actual_owner_side_mutation_authority_class_choice_status",
    "stack_command_implementation_actual_owner_side_mutation_authority_class_choice_question",
    "stack_command_implementation_actual_owner_side_mutation_authority_class_choice_reasons",
)
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE_QUESTION_PROMPT
        ...
        == list(STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE_CONTRACT_RECEIPT_REFS)
    )
...
def _attempt_reasons(bundle: Mapping[str, Any]) -> list[str]:
    ...
    checks = (
        (
            ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_VALUE_KEYS,
            "actual_owner_side_mutation_authority_class_value_attempted",
        ),
        (
            ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_KEYS,
            "actual_owner_side_mutation_authority_class_value_attempted",
        ),
        (ACTUAL_OWNER_SIDE_MUTATION_SURFACE_KEYS, "actual_owner_side_mutation_surface_attempted"),
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_class_choice_status"))
        != STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE_STATUS_ADMISSIBLE
    ):
        return ["stack_command_implementation_actual_owner_side_mutation_authority_class_choice_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_class_choice_question"),
        candidate_path,
    ):
        reasons.append("stack_command_implementation_actual_owner_side_mutation_authority_class_choice_question_not_explicit")
    if _normalized_string_list(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_class_choice_reasons")
    ):
        reasons.append("stack_command_implementation_actual_owner_side_mutation_authority_class_choice_reasons_present")
...
def evaluate_supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_authority_class_value(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    result["stack_command_implementation_actual_owner_side_mutation_authority_class_value_status"] = (
        STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_VALUE_STATUS_ADMISSIBLE
        if not reasons
        else NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_VALUE
    )
    result["stack_command_implementation_actual_owner_side_mutation_authority_class_value_question"] = (
        _question_card(candidate_path) if not reasons else None
    )
    result["stack_command_implementation_actual_owner_side_mutation_authority_class_value_reasons"] = reasons
    return result
