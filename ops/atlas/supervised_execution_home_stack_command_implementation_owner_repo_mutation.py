...
from ops.atlas.supervised_execution_home_stack_command_implementation_owner_repo_implementation_routing_card import (
    CONTRACT_RECEIPT_REFS as STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD_CONTRACT_RECEIPT_REFS,
    PASSTHROUGH_FIELDS as STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD_PASSTHROUGH_FIELDS,
    QUESTION_PROMPT as STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD_QUESTION_PROMPT,
...
STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_MUTATION_STATUS_ADMISSIBLE = (
    "stack_command_implementation_owner_repo_mutation_admissible"
)
NO_STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_MUTATION = (
    "no_stack_command_implementation_owner_repo_mutation"
)
QUESTION_PROMPT = (
    "Which owner-repo mutation contract, if any, may any later admitted "
    "surface define for the already-admitted _stack command implementation "
    "owner-repo implementation-routing card seam for `stack "
    "supervised-execution-home` without defining one actual owner-repo "
    "mutation surface, one actual owner-side mutation authority, one actual "
    "concrete command file, one actual downstream runtime-home value "
    "placement, one actual concrete _stack command implementation surface "
    "choice, one actual _stack command implementation, one Playbook doctrine "
    "export path, deploy or publication work, or one protected-surface touch?"
)
...
PASSTHROUGH_FIELDS = (
    *STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD_PASSTHROUGH_FIELDS,
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD_QUESTION_PROMPT
        ...
        == list(STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("stack_command_implementation_owner_repo_implementation_routing_card_status"))
        != STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD_STATUS_ADMISSIBLE
    ):
        return ["stack_command_implementation_owner_repo_implementation_routing_card_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("stack_command_implementation_owner_repo_implementation_routing_card_question"),
        candidate_path,
    ):
        reasons.append(
            "stack_command_implementation_owner_repo_implementation_routing_card_question_not_explicit"
        )
    if _normalized_string_list(
        bundle.get("stack_command_implementation_owner_repo_implementation_routing_card_reasons")
    ):
        reasons.append(
            "stack_command_implementation_owner_repo_implementation_routing_card_reasons_present"
        )
...
def evaluate_supervised_execution_home_stack_command_implementation_owner_repo_mutation(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    result["stack_command_implementation_owner_repo_mutation_status"] = (
        STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_MUTATION_STATUS_ADMISSIBLE
        if not reasons
        else NO_STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_MUTATION
    )
    result["stack_command_implementation_owner_repo_mutation_question"] = (
        _question_card(candidate_path) if not reasons else None
    )
    result["stack_command_implementation_owner_repo_mutation_reasons"] = reasons
    return result
