...
from ops.atlas.supervised_execution_home_stack_command_implementation_owner_repo_implementation_routing import (
    ...
    PASSTHROUGH_FIELDS as STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_PASSTHROUGH_FIELDS,
...
STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD_STATUS_ADMISSIBLE = (
    "stack_command_implementation_owner_repo_implementation_routing_card_admissible"
)
NO_STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD = (
    "no_stack_command_implementation_owner_repo_implementation_routing_card"
)
QUESTION_PROMPT = (
    "Which owner-repo implementation-routing card contract, if any, may any "
    "later admitted surface define for the already-admitted _stack command "
    "implementation owner-repo implementation-routing seam for `stack "
    "supervised-execution-home` without defining one actual owner-repo "
    "implementation-routing card, one actual owner-repo mutation surface, one "
    "actual owner-side mutation authority, one actual `_stack` command "
    "implementation, one actual concrete command file, one actual downstream "
    "runtime-home value placement, one actual concrete `_stack` command "
    "implementation-surface choice, one Playbook doctrine export path, deploy "
    "or publication work, or protected-surface touch?"
)
...
ACTUAL_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD_KEYS = {
    "actual_owner_repo_implementation_routing_card",
    "implementation_routing_card",
    "owner_repo_implementation_routing_card",
    "routing_card",
    "routing_card_contract",
}
OWNER_REPO_MUTATION_SURFACE_KEYS = {
    "mutation_surface",
...
PASSTHROUGH_FIELDS = (
    *STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_PASSTHROUGH_FIELDS,
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_QUESTION_PROMPT
        ...
        == list(STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CONTRACT_RECEIPT_REFS)
    )
...
def _attempt_reasons(bundle: Mapping[str, Any]) -> list[str]:
    ...
    checks = (
        ...
        (OWNER_REPO_MUTATION_SURFACE_KEYS, "owner_repo_mutation_surface_attempted"),
        (
            ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_KEYS,
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("stack_command_implementation_owner_repo_implementation_routing_status"))
        != STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_STATUS_ADMISSIBLE
    ):
        return ["stack_command_implementation_owner_repo_implementation_routing_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("stack_command_implementation_owner_repo_implementation_routing_question"),
        candidate_path,
    ):
        reasons.append("stack_command_implementation_owner_repo_implementation_routing_question_not_explicit")
    if _normalized_string_list(
        bundle.get("stack_command_implementation_owner_repo_implementation_routing_reasons")
    ):
        reasons.append("stack_command_implementation_owner_repo_implementation_routing_reasons_present")
...
def evaluate_supervised_execution_home_stack_command_implementation_owner_repo_implementation_routing_card(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    result["stack_command_implementation_owner_repo_implementation_routing_card_status"] = (
        STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD_STATUS_ADMISSIBLE
        if not reasons
        else NO_STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_CARD
    )
    result["stack_command_implementation_owner_repo_implementation_routing_card_question"] = (
        _question_card(candidate_path) if not reasons else None
    )
    result["stack_command_implementation_owner_repo_implementation_routing_card_reasons"] = reasons
    return result
