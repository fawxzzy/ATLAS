...
from ops.atlas.supervised_execution_home_concrete_command_file import (
    ...
    CONTRACT_RECEIPT_REFS as CONCRETE_COMMAND_FILE_SELECTION_CONTRACT_RECEIPT_REFS,
    QUESTION_PROMPT as CONCRETE_COMMAND_FILE_SELECTION_QUESTION_PROMPT,
)
...
NO_CONCRETE_COMMAND_FILE_CHOICE = "no_concrete_command_file_choice"
QUESTION_PROMPT = (
    "Which concrete command file, if any, may carry the downstream "
    "actual-concrete-runtime-home-value-placement and command-runtime "
    "implementation chain for the already-admitted concrete `_stack` command-home "
    "choice seam for `stack supervised-execution-home` without choosing it now or "
    "widening into actual concrete runtime-home value placement, concrete `_stack` "
    "command implementation-surface choice, `_stack` implementation, worker "
    "authority, owner-repo edits, actual owner-side mutation authority, Playbook "
    "doctrine export, or protected-surface exceptions?"
)
...
CONCRETE_COMMAND_FILE_CHOICE_KEYS = {
    "command_file",
    "command_file_choice",
    "command_file_path",
    "concrete_command_file",
    "concrete_command_file_choice",
...
CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_KEYS = {
...
def _question_card_is_explicit(
    value: Any,
    candidate_path: str,
    ...
    return (
        ...
        and _normalized_text(question_card.get("question")) == expected_question
...
def _attempt_reasons(bundle: Mapping[str, Any]) -> list[str]:
    ...
    checks = (
        ...
        (CONCRETE_COMMAND_FILE_CHOICE_KEYS, "concrete_command_file_choice_attempted"),
        (
            ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_PLACEMENT_KEYS,
...
def _selection_reasons(
    ...
    if (
        ...
        return ["concrete_command_file_selection_status_not_admissible"]

    if (
        _normalized_text(bundle.get("concrete_stack_command_home_choice_status"))
        != CONCRETE_STACK_COMMAND_HOME_CHOICE_STATUS_ADMISSIBLE
    ):
        return ["concrete_stack_command_home_choice_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("concrete_command_file_selection_question"),
        candidate_path,
        CONCRETE_COMMAND_FILE_SELECTION_QUESTION_PROMPT,
        CONCRETE_COMMAND_FILE_SELECTION_CONTRACT_RECEIPT_REFS,
    ):
    ...
    if not _question_card_is_explicit(
        bundle.get("concrete_stack_command_home_choice_question"),
        candidate_path,
        CONCRETE_STACK_COMMAND_HOME_CHOICE_QUESTION_PROMPT,
        CONCRETE_STACK_COMMAND_HOME_CHOICE_CONTRACT_RECEIPT_REFS,
    ):
        reasons.append("concrete_stack_command_home_choice_question_not_explicit")
    if _normalized_string_list(bundle.get("concrete_stack_command_home_choice_reasons")):
        reasons.append("concrete_stack_command_home_choice_reasons_present")
...
def evaluate_supervised_execution_home_concrete_command_file_choice(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    return {
        ...
        "concrete_command_file_choice_status": (
            CONCRETE_COMMAND_FILE_CHOICE_STATUS_ADMISSIBLE
            if not reasons
            else NO_CONCRETE_COMMAND_FILE_CHOICE
        ),
        "concrete_command_file_choice_question": (
            _question_card(candidate_path) if not reasons else None
        ),
        "concrete_command_file_choice_reasons": reasons,
    }
