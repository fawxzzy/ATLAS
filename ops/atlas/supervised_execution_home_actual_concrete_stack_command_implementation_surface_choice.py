...
from ops.atlas.supervised_execution_home_concrete_stack_command_implementation_surface_choice import (
    ...
    CONTRACT_RECEIPT_REFS as CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE_CONTRACT_RECEIPT_REFS,
    QUESTION_PROMPT as CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE_QUESTION_PROMPT,
)

ACTUAL_CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE_STATUS_ADMISSIBLE = (
    "actual_concrete_stack_command_implementation_surface_choice_admissible"
)
NO_ACTUAL_CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE = (
    "no_actual_concrete_stack_command_implementation_surface_choice"
)
QUESTION_PROMPT = (
    "Which actual concrete _stack command implementation surface, if any, may any "
    "later admitted surface choose for the already-admitted concrete _stack command "
    "implementation-surface choice seam for `stack supervised-execution-home` to "
    "carry the already-admitted actual concrete command-file downstream runtime-home "
    "value placement seam without choosing it now or widening into one actual "
    "concrete command file, one actual downstream runtime-home value placement, one "
    "_stack command implementation, one worker authority class, one owner-repo edit "
    "authority, one actual owner-side mutation authority, one Playbook doctrine "
    "export path, or one protected-surface exception?"
)
CONTRACT_RECEIPT_REFS = (
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-ACTUAL-CONCRETE-STACK-COMMAND-IMPLEMENTATION-SURFACE-CHOICE-CONTRACT-FREEZE-PASS-652-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-ACTUAL-CONCRETE-STACK-COMMAND-IMPLEMENTATION-SURFACE-CHOICE-OWNER-SURFACE-ADMISSION-PASS-653-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-ACTUAL-CONCRETE-STACK-COMMAND-IMPLEMENTATION-SURFACE-CHOICE-SUPPORTING-LANE-ADMISSION-PASS-654-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-ACTUAL-CONCRETE-STACK-COMMAND-IMPLEMENTATION-SURFACE-CHOICE-FIRST-IMPLEMENTATION-ADMISSION-PASS-655-2026-06-25.md",
)
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE_QUESTION_PROMPT
        ...
        == list(CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(
            bundle.get("concrete_stack_command_implementation_surface_choice_status")
        )
        != CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE_STATUS_ADMISSIBLE
    ):
        return ["concrete_stack_command_implementation_surface_choice_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("concrete_stack_command_implementation_surface_choice_question"),
        candidate_path,
    ):
        reasons.append(
            "concrete_stack_command_implementation_surface_choice_question_not_explicit"
        )
    if _normalized_string_list(
        bundle.get("concrete_stack_command_implementation_surface_choice_reasons")
    ):
        reasons.append(
            "concrete_stack_command_implementation_surface_choice_reasons_present"
        )
...
def evaluate_supervised_execution_home_actual_concrete_stack_command_implementation_surface_choice(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    concrete_stack_command_implementation_surface_choice_question = bundle.get(
    ...
    return {
        ...
        "concrete_stack_command_implementation_surface_selection_question": (
            ...
            if isinstance(
                concrete_stack_command_implementation_surface_selection_question, Mapping
            )
            else None
        ),
        "concrete_stack_command_implementation_surface_selection_reasons": _normalized_string_list(
            bundle.get("concrete_stack_command_implementation_surface_selection_reasons")
        ),
        "concrete_stack_command_implementation_surface_choice_status": _normalized_text(
            bundle.get("concrete_stack_command_implementation_surface_choice_status")
        ),
        "concrete_stack_command_implementation_surface_choice_question": (
            dict(concrete_stack_command_implementation_surface_choice_question)
            if isinstance(
                concrete_stack_command_implementation_surface_choice_question, Mapping
            )
            else None
        ),
        "concrete_stack_command_implementation_surface_choice_reasons": _normalized_string_list(
            bundle.get("concrete_stack_command_implementation_surface_choice_reasons")
        ),
        "actual_concrete_stack_command_implementation_surface_choice_status": (
            ACTUAL_CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE_STATUS_ADMISSIBLE
            if not reasons
            else NO_ACTUAL_CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE
        ),
        "actual_concrete_stack_command_implementation_surface_choice_question": (
            _question_card(candidate_path) if not reasons else None
        ),
        "actual_concrete_stack_command_implementation_surface_choice_reasons": reasons,
    }
