...
from ops.atlas.supervised_execution_home_concrete_stack_command_home import (
    ...
    CONTRACT_RECEIPT_REFS as CONCRETE_STACK_COMMAND_HOME_SELECTION_CONTRACT_RECEIPT_REFS,
...
CONCRETE_STACK_COMMAND_HOME_CHOICE_STATUS_ADMISSIBLE = (
    "concrete_stack_command_home_choice_admissible"
)
NO_CONCRETE_STACK_COMMAND_HOME_CHOICE = "no_concrete_stack_command_home_choice"
QUESTION_PROMPT = (
    "Which concrete _stack command home, if any, may carry the downstream "
    "actual-concrete-runtime-home-value-placement and command-runtime implementation "
    "chain for `stack supervised-execution-home` without choosing it now or widening "
    "into actual concrete runtime-home value placement, concrete command-file choice, "
    "concrete implementation-surface choice, `_stack` implementation, worker authority, "
    "owner-repo edits, actual owner-side mutation authority, Playbook doctrine export, "
    "or protected-surface exceptions?"
)
CONTRACT_RECEIPT_REFS = (
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-CONCRETE-STACK-COMMAND-HOME-CHOICE-CONTRACT-FREEZE-PASS-609-2026-06-24.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-CONCRETE-STACK-COMMAND-HOME-CHOICE-OWNER-SURFACE-ADMISSION-PASS-610-2026-06-24.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-CONCRETE-STACK-COMMAND-HOME-CHOICE-SUPPORTING-LANE-ADMISSION-PASS-611-2026-06-24.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-CONCRETE-STACK-COMMAND-HOME-CHOICE-FIRST-IMPLEMENTATION-ADMISSION-PASS-612-2026-06-24.md",
)
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == CONCRETE_STACK_COMMAND_HOME_SELECTION_QUESTION_PROMPT
        ...
        == list(CONCRETE_STACK_COMMAND_HOME_SELECTION_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("concrete_stack_command_home_selection_status"))
        != CONCRETE_STACK_COMMAND_HOME_SELECTION_STATUS_ADMISSIBLE
    ):
        return ["concrete_stack_command_home_selection_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("concrete_stack_command_home_selection_question"),
        candidate_path,
    ):
        reasons.append("concrete_stack_command_home_selection_question_not_explicit")
    if _normalized_string_list(bundle.get("concrete_stack_command_home_selection_reasons")):
        reasons.append("concrete_stack_command_home_selection_reasons_present")
...
def evaluate_supervised_execution_home_concrete_stack_command_home_choice(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
