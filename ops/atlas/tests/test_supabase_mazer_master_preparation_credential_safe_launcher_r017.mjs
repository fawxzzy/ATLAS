...
if ([source, manifest, predecessor, host].every(file => fs.existsSync(file))) {
  ...
  const lineage = (name, { aliasIssued, authorized, consumed, expires, envelopeIssued = new Date(), notBefore = consumed, successorExists = false, tuple = baseTuple, decisionExpiresRaw, aliasExpiresRaw, approvalReferenceMode = 'canonical' }) => {
