...
for (const token of [
  ...
  '$priorFenceRestored', "'PREPARATION_COMPLETE'", "'HOST_EXACT_ROLLBACK_COMPLETED_AND_LEGACY_RELEASED'",
...
if ([source, manifest, predecessor, host].every(file => fs.existsSync(file))) {
  ...
  const restoredPredecessor=path.join(runtime,`mazer-master-r017-terminal-rollback-preparation-complete-${crypto.randomUUID()}.json`), restoredValue={...JSON.parse(fs.readFileSync(predecessor,'utf8')),fence_phase:'PREPARATION_COMPLETE',postrestore_proof:'HOST_EXACT_ROLLBACK_COMPLETED_AND_LEGACY_RELEASED'}, restoredWritten=write(restoredPredecessor,restoredValue), restoredTuple={...evolutionTuple,predecessor:restoredPredecessor,predecessorSha:restoredWritten.sha};
  const unprovenPredecessor=path.join(runtime,`mazer-master-r017-terminal-rollback-preparation-unproven-${crypto.randomUUID()}.json`), unprovenWritten=write(unprovenPredecessor,{...restoredValue,postrestore_proof:'UNPROVEN'}), unprovenTuple={...evolutionTuple,predecessor:unprovenPredecessor,predecessorSha:unprovenWritten.sha};
  ...
  try {
    for(const shell of ['pwsh.exe','powershell.exe']){
      ...
      const restored=lineage(`restored-preparation-complete-${shell}`,{aliasIssued:new Date(now-2*3600000),authorized:new Date(now-120000),consumed:new Date(now-60000),expires:new Date(now.getTime()+3600000),envelopeIssued:now,tuple:restoredTuple});
      assert.equal(validate(shell,'restored-preparation-complete',restored.envelope,0).receipt.result,'PASS_R017_LAUNCHER_INVOCATION_BOUND');
      const unproven=lineage(`unproven-preparation-complete-${shell}`,{aliasIssued:new Date(now-2*3600000),authorized:new Date(now-120000),consumed:new Date(now-60000),expires:new Date(now.getTime()+3600000),envelopeIssued:now,tuple:unprovenTuple});
      validate(shell,'unproven-preparation-complete',unproven.envelope,2,'PREDECESSOR_EFFECT_STATE');
