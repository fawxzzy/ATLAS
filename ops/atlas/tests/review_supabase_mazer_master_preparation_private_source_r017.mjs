...
for (const token of [
  'transaction isolation level serializable read only','PGDATABASE: databaseUrl','PRIVATE_OUTPUT_MUST_BE_UNDER_SECRETS',"flag: 'wx'",
...
for (const token of ['source_snapshot?.ai','target_snapshot?.ai','source_snapshot?.player','target_snapshot?.player']) requireToken(materializer, token, `RESET_BINDING_PATH_MISSING:${token}`);
for (const token of ['identity_edges: 18','sql_programs: 9','materializer main executed more than once','NORMALIZED_EMAIL_DUPLICATE','PRIVATE_SECRET_INPUT_WEAK']) requireToken(focused, token, `ADVERSARY_MISSING:${token}`);
