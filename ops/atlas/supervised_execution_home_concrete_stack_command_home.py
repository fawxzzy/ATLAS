...
CONCRETE_STACK_COMMAND_HOME_SELECTION_STATUS_ADMISSIBLE = (
    "concrete_stack_command_home_selection_admissible"
)
NO_CONCRETE_STACK_COMMAND_HOME_SELECTION = "no_concrete_stack_command_home_selection"
QUESTION_PROMPT = (
    ...
    "runtime home, one _stack command implementation surface, one worker authority class, "
    "one owner-repo edit authority, one actual owner-side mutation authority, one Playbook "
...
def _bundle_posture_is_explicit(bundle: Mapping[str, Any], candidate_path: str, payload: dict[str, Any]) -> bool:
    return (
...
def _attempt_reasons(bundle: Mapping[str, Any]) -> list[str]:
    reasons: list[str] = []
    checks = (
        (CONCRETE_STACK_COMMAND_HOME_KEYS, "concrete_stack_command_home_choice_attempted"),
...
def _selection_reasons(bundle: Mapping[str, Any], candidate_path: str, payload: dict[str, Any]) -> list[str]:
    reasons: list[str] = []
    if _normalized_text(bundle.get("concrete_command_home_status")) != CONCRETE_COMMAND_HOME_STATUS_ADMISSIBLE:
        reasons.append("concrete_command_home_status_not_admissible")
    if not _question_card_is_explicit(bundle.get("concrete_command_home_question"), candidate_path):
        reasons.append("concrete_command_home_question_not_explicit")
    if _normalized_string_list(bundle.get("concrete_command_home_reasons")):
        reasons.append("concrete_command_home_reasons_present")
...
def evaluate_supervised_execution_home_concrete_stack_command_home(
    ...
    return {
        ...
        "concrete_command_home_status": _normalized_text(bundle.get("concrete_command_home_status")),
