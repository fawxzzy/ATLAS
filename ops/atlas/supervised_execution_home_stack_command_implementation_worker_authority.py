...
from ops.atlas.supervised_execution_home_stack_command_implementation import (
    ...
    PASSTHROUGH_FIELDS as STACK_COMMAND_IMPLEMENTATION_PASSTHROUGH_FIELDS,
...
STACK_COMMAND_IMPLEMENTATION_WORKER_AUTHORITY_STATUS_ADMISSIBLE = (
    "stack_command_implementation_worker_authority_admissible"
)
NO_STACK_COMMAND_IMPLEMENTATION_WORKER_AUTHORITY = (
    "no_stack_command_implementation_worker_authority"
)
QUESTION_PROMPT = (
    "Which worker-authority contract, if any, may any later admitted surface "
    "define for the already-admitted _stack command implementation seam for "
    "`stack supervised-execution-home` without defining one actual worker "
    "authority class now or widening into one actual concrete command file, one "
    "actual downstream runtime-home value placement, one actual concrete _stack "
    "command implementation surface choice, one actual _stack command "
    "implementation, one owner-repo implementation-routing boundary, one actual "
    "owner-side mutation authority, one Playbook doctrine export path, or one "
    "protected-surface exception?"
)
CONTRACT_RECEIPT_REFS = (
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-WORKER-AUTHORITY-CONTRACT-FREEZE-PASS-667-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-WORKER-AUTHORITY-OWNER-SURFACE-ADMISSION-PASS-668-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-WORKER-AUTHORITY-SUPPORTING-LANE-ADMISSION-PASS-669-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-WORKER-AUTHORITY-FIRST-IMPLEMENTATION-ADMISSION-PASS-670-2026-06-25.md",
)
...
PASSTHROUGH_FIELDS = (
    *STACK_COMMAND_IMPLEMENTATION_PASSTHROUGH_FIELDS,
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == STACK_COMMAND_IMPLEMENTATION_QUESTION_PROMPT
        ...
        == list(STACK_COMMAND_IMPLEMENTATION_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("stack_command_implementation_status"))
        != STACK_COMMAND_IMPLEMENTATION_STATUS_ADMISSIBLE
    ):
        return ["stack_command_implementation_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("stack_command_implementation_question"),
        candidate_path,
    ):
        reasons.append("stack_command_implementation_question_not_explicit")
    if _normalized_string_list(bundle.get("stack_command_implementation_reasons")):
        reasons.append("stack_command_implementation_reasons_present")
...
def evaluate_supervised_execution_home_stack_command_implementation_worker_authority(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    result["stack_command_implementation_worker_authority_status"] = (
        STACK_COMMAND_IMPLEMENTATION_WORKER_AUTHORITY_STATUS_ADMISSIBLE
        if not reasons
        else NO_STACK_COMMAND_IMPLEMENTATION_WORKER_AUTHORITY
    )
    result["stack_command_implementation_worker_authority_question"] = (
        _question_card(candidate_path) if not reasons else None
    )
    result["stack_command_implementation_worker_authority_reasons"] = reasons
    return result
