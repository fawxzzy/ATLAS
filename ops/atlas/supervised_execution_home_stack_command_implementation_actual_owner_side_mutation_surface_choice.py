...
from ops.atlas.supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_authority_class_realization import (
    ...
    PASSTHROUGH_FIELDS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_REALIZATION_PASSTHROUGH_FIELDS,
...
STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE_STATUS_ADMISSIBLE = (
    "stack_command_implementation_actual_owner_side_mutation_surface_choice_admissible"
)
NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE = (
    "no_stack_command_implementation_actual_owner_side_mutation_surface_choice"
)
QUESTION_PROMPT = (
    "Which actual owner-side mutation surface, if any, may any later admitted "
    "surface choose for the already-admitted _stack command implementation "
    "actual owner-side mutation surface seam, the now-bounded actual owner-side "
    "mutation authority-class seam, the now-bounded actual owner-side mutation "
    "surface-realization seam, and the now-bounded actual owner-side mutation "
    "authority-class-realization seam for `stack supervised-execution-home` "
    "without choosing it now, one actual owner-side mutation authority class "
    "now, one actual `_stack` execution-home surface, one actual concrete "
...
CONTRACT_RECEIPT_REFS = (
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-SURFACE-CHOICE-CONTRACT-FREEZE-PASS-741-2026-06-26.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-SURFACE-CHOICE-OWNER-SURFACE-ADMISSION-PASS-742-2026-06-26.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-SURFACE-CHOICE-SUPPORTING-LANE-ADMISSION-PASS-743-2026-06-26.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-SURFACE-CHOICE-FIRST-IMPLEMENTATION-ADMISSION-PASS-744-2026-06-26.md",
)
...
PASSTHROUGH_FIELDS = (
    *STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_REALIZATION_PASSTHROUGH_FIELDS,
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == list(STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_REALIZATION_CONTRACT_RECEIPT_REFS)
    )
...
def _attempt_reasons(bundle: Mapping[str, Any]) -> list[str]:
    reasons: list[str] = []
    checks = (
        (ACTUAL_OWNER_SIDE_MUTATION_SURFACE_KEYS, "actual_owner_side_mutation_surface_choice_attempted"),
        (ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_KEYS, "actual_owner_side_mutation_authority_class_attempted"),
...
def _selection_reasons(
    ...
    if (
        _normalized_text(
            bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_class_realization_status")
        )
        != STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_REALIZATION_STATUS_ADMISSIBLE
    ):
        return ["stack_command_implementation_actual_owner_side_mutation_authority_class_realization_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_class_realization_question"),
        candidate_path,
    ):
        reasons.append(
    ...
    if _normalized_string_list(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_class_realization_reasons")
    ):
        reasons.append(
...
def evaluate_supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_surface_choice(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    result["stack_command_implementation_actual_owner_side_mutation_surface_choice_status"] = (
        STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE_STATUS_ADMISSIBLE
        if not reasons
        else NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE
    )
    result["stack_command_implementation_actual_owner_side_mutation_surface_choice_question"] = (
        _question_card(candidate_path) if not reasons else None
    )
    result["stack_command_implementation_actual_owner_side_mutation_surface_choice_reasons"] = reasons
    return result
