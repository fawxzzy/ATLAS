...
NO_ACTUAL_CONCRETE_RUNTIME_HOME_VALUE = "no_actual_concrete_runtime_home_value"
QUESTION_PROMPT = (
    "Which actual concrete runtime-home value, if any, may any later admitted surface "
    "choose for the already-admitted actual-concrete-runtime-home-value-choice seam "
    "for `stack supervised-execution-home` without choosing one actual concrete "
    "runtime-home value now or widening into concrete implementation-surface choice, "
    "`_stack` implementation, worker authority, owner-repo edits, actual owner-side "
    "mutation authority, Playbook doctrine export, or protected-surface exceptions?"
)
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_CHOICE_QUESTION_PROMPT
        ...
        == list(ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_CHOICE_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("actual_concrete_runtime_home_value_choice_status"))
        != ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_CHOICE_STATUS_ADMISSIBLE
    ):
        return ["actual_concrete_runtime_home_value_choice_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("actual_concrete_runtime_home_value_choice_question"), candidate_path
    ):
        reasons.append("actual_concrete_runtime_home_value_choice_question_not_explicit")
    if _normalized_string_list(bundle.get("actual_concrete_runtime_home_value_choice_reasons")):
        reasons.append("actual_concrete_runtime_home_value_choice_reasons_present")
...
def evaluate_supervised_execution_home_actual_concrete_runtime_home_value(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
