...
def _validate_decision_ref(value: str) -> tuple[str, OrderedDict[str, Any] | None]:
    normalized = _normalized_relative(value)
    protected_prefixes = ("repos/", "secrets/", "runtime/", "tmp/", ".vercel/", "archive/", ".playwright-mcp/")
    if normalized.startswith(protected_prefixes):
        return normalized, _finding("unsupported_decision_ref", "Decision ref must be a durable root-owned docs surface.", severity="blocker", decision_ref=normalized)
    if not normalized.startswith("docs/"):
        return normalized, _finding("unsupported_decision_ref", "Decision ref must be under docs/**.", severity="blocker", decision_ref=normalized)
    return normalized, None


def build_report(*, root: Path, candidate_id: str, decision_ref: str, review_report_path: str | None = None) -> OrderedDict[str, Any]:
    ...
    normalized_decision_ref, decision_error = _validate_decision_ref(decision_ref)
    if decision_error is not None:
        blockers.append(decision_error)
    ...
    if review_report is not None:
        ...
        elif normalized_candidate_id:
            selected_review = _review_by_candidate(review_report, normalized_candidate_id)
            if selected_review is None:
                warnings.append(_finding("candidate_not_found", "Requested candidate was not present in the review report.", candidate_id=normalized_candidate_id))
            elif str(selected_review.get("review_status") or "") != "review_ready":
                blockers.append(_finding("candidate_not_review_ready", "Requested candidate is not review-ready.", severity="blocker", candidate_id=normalized_candidate_id, review_status=str(selected_review.get("review_status") or "")))
            else:
                packet_ladder = _stage_payload(normalized_candidate_id)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Package an accepted receipt-derived automation candidate into a deterministic packet ladder.")
    ...
    parser.add_argument("--candidate-id", required=True, help="Candidate id to package.")
    parser.add_argument("--decision-ref", required=True, help="Root-relative docs/** decision receipt ref.")
    ...
    try:
        ...
        report = OrderedDict(
            [
                ...
                ("boundaries", BOUNDARIES),
                ("warnings", []),
                ("blockers", [_finding("internal_error", "Automation candidate packet ladder packaging failed before classification.", severity="blocker", exception=str(exc))]),
                ("safe_to_use", False),
            ]
        )
