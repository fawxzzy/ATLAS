...
ACTUAL_CONCRETE_COMMAND_FILE_DOWNSTREAM_RUNTIME_HOME_VALUE_PLACEMENT_STATUS_ADMISSIBLE = (
    "actual_concrete_command_file_downstream_runtime_home_value_placement_admissible"
)
NO_ACTUAL_CONCRETE_COMMAND_FILE_DOWNSTREAM_RUNTIME_HOME_VALUE_PLACEMENT = (
    "no_actual_concrete_command_file_downstream_runtime_home_value_placement"
)
QUESTION_PROMPT = (
    "Where, if anywhere, may any later admitted surface place the downstream "
    "runtime-home value for the already-admitted actual concrete command-file "
    "choice seam for `stack supervised-execution-home` without placing it now "
    "or widening into one actual concrete command file, one concrete `_stack` "
    "command implementation surface, `_stack` implementation, worker authority, "
    "owner-repo edits, actual owner-side mutation authority, Playbook doctrine "
    "export, or protected-surface exceptions?"
)
...
ACTUAL_CONCRETE_COMMAND_FILE_DOWNSTREAM_RUNTIME_HOME_VALUE_PLACEMENT_KEYS = {
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == ACTUAL_CONCRETE_COMMAND_FILE_CHOICE_QUESTION_PROMPT
        ...
        == list(ACTUAL_CONCRETE_COMMAND_FILE_CHOICE_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("actual_concrete_command_file_choice_status"))
        != ACTUAL_CONCRETE_COMMAND_FILE_CHOICE_STATUS_ADMISSIBLE
    ):
        return ["actual_concrete_command_file_choice_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("actual_concrete_command_file_choice_question"), candidate_path
    ):
        reasons.append("actual_concrete_command_file_choice_question_not_explicit")
    if _normalized_string_list(bundle.get("actual_concrete_command_file_choice_reasons")):
        reasons.append("actual_concrete_command_file_choice_reasons_present")
...
def evaluate_supervised_execution_home_actual_concrete_command_file_downstream_runtime_home_value_placement(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    return {
        ...
        "actual_concrete_command_file_downstream_runtime_home_value_placement_status": (
            ACTUAL_CONCRETE_COMMAND_FILE_DOWNSTREAM_RUNTIME_HOME_VALUE_PLACEMENT_STATUS_ADMISSIBLE
            if not reasons
            else NO_ACTUAL_CONCRETE_COMMAND_FILE_DOWNSTREAM_RUNTIME_HOME_VALUE_PLACEMENT
        ),
        "actual_concrete_command_file_downstream_runtime_home_value_placement_question": (
            _question_card(candidate_path) if not reasons else None
        ),
        "actual_concrete_command_file_downstream_runtime_home_value_placement_reasons": reasons,
    }
