...
RUNTIME_HOME_SELECTION_STATUS_ADMISSIBLE = "runtime_home_selection_admissible"
NO_RUNTIME_HOME_SELECTION = "no_runtime_home_selection"
QUESTION_PROMPT = (
    "Which later admitted surface, if any, may choose one runtime home for the already-admitted "
    "concrete _stack command implementation-surface seam for `stack supervised-execution-home` "
    "without choosing one runtime home now or widening into concrete implementation-surface "
    "choice, _stack implementation, worker authority, owner-repo edits, actual owner-side "
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_QUESTION_PROMPT
        ...
        == list(CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CONTRACT_RECEIPT_REFS)
    )
...
def evaluate_supervised_execution_home_runtime_home_selection(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_home_selection_status": (
            ...
            if not reasons
            else NO_RUNTIME_HOME_SELECTION
        ),
        "runtime_home_selection_question": _question_card(candidate_path) if not reasons else None,
        "runtime_home_selection_reasons": reasons,
    }
