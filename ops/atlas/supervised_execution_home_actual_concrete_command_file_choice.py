...
ACTUAL_CONCRETE_COMMAND_FILE_CHOICE_STATUS_ADMISSIBLE = (
    "actual_concrete_command_file_choice_admissible"
)
NO_ACTUAL_CONCRETE_COMMAND_FILE_CHOICE = "no_actual_concrete_command_file_choice"
QUESTION_PROMPT = (
    "Which actual concrete command file, if any, may any later admitted surface "
    "choose for the already-admitted concrete command-file choice seam for `stack "
    "supervised-execution-home` to carry the downstream actual-concrete-runtime-"
    "home-value-placement and command-runtime implementation chain without "
    "choosing it now or widening into actual concrete runtime-home value "
    "placement, concrete `_stack` command implementation-surface choice, "
    "`_stack` implementation, worker authority, owner-repo edits, actual "
    "owner-side mutation authority, Playbook doctrine export, or protected-"
    "surface exceptions?"
)
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == CONCRETE_COMMAND_FILE_CHOICE_QUESTION_PROMPT
        ...
        == list(CONCRETE_COMMAND_FILE_CHOICE_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("concrete_command_file_choice_status"))
        != CONCRETE_COMMAND_FILE_CHOICE_STATUS_ADMISSIBLE
    ):
        return ["concrete_command_file_choice_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("concrete_command_file_choice_question"), candidate_path
    ):
        reasons.append("concrete_command_file_choice_question_not_explicit")
    if _normalized_string_list(bundle.get("concrete_command_file_choice_reasons")):
        reasons.append("concrete_command_file_choice_reasons_present")
...
def evaluate_supervised_execution_home_actual_concrete_command_file_choice(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    return {
        ...
        "actual_concrete_command_file_choice_status": (
            ACTUAL_CONCRETE_COMMAND_FILE_CHOICE_STATUS_ADMISSIBLE
            if not reasons
            else NO_ACTUAL_CONCRETE_COMMAND_FILE_CHOICE
        ),
        "actual_concrete_command_file_choice_question": (
            _question_card(candidate_path) if not reasons else None
        ),
        "actual_concrete_command_file_choice_reasons": reasons,
    }
