...
from ops.atlas.supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_authority_class import (
    ...
    CONTRACT_RECEIPT_REFS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CONTRACT_RECEIPT_REFS,
    LIVE_REPO_OR_TRANSCRIPT_KEYS,
    PASSTHROUGH_FIELDS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_PASSTHROUGH_FIELDS,
    PLAYBOOK_DOCTRINE_EXPORT_KEYS,
    PROTECTED_SURFACE_EXCEPTION_KEYS,
    QUESTION_PROMPT as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_QUESTION_PROMPT,
...
STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_REALIZATION_STATUS_ADMISSIBLE = (
    "stack_command_implementation_actual_owner_side_mutation_surface_realization_admissible"
)
...
QUESTION_PROMPT = (
    ...
    "mutation surface allowed by the already-admitted _stack command "
    "implementation actual owner-side mutation surface seam and now-bounded "
    "actual owner-side mutation authority-class seam for `stack "
    "supervised-execution-home` without realizing one actual owner-side "
    "mutation surface now, defining one actual owner-side mutation authority "
...
PASSTHROUGH_FIELDS = (
    *STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_PASSTHROUGH_FIELDS,
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_QUESTION_PROMPT
        ...
        == list(STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(
            bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_class_status")
        )
        != STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_STATUS_ADMISSIBLE
    ):
        return ["stack_command_implementation_actual_owner_side_mutation_authority_class_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_class_question"),
        candidate_path,
    ):
        reasons.append(
            "stack_command_implementation_actual_owner_side_mutation_authority_class_question_not_explicit"
        )
    if _normalized_string_list(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_class_reasons")
    ):
        reasons.append(
            "stack_command_implementation_actual_owner_side_mutation_authority_class_reasons_present"
        )
...
def evaluate_supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_surface_realization(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    result["stack_command_implementation_actual_owner_side_mutation_surface_realization_status"] = (
        STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_REALIZATION_STATUS_ADMISSIBLE
        if not reasons
        else NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_REALIZATION
    )
    result["stack_command_implementation_actual_owner_side_mutation_surface_realization_question"] = (
        _question_card(candidate_path) if not reasons else None
    )
    result["stack_command_implementation_actual_owner_side_mutation_surface_realization_reasons"] = reasons
    return result
