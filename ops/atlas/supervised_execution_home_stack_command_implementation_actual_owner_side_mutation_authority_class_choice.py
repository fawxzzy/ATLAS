...
from ops.atlas.supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_surface_choice import (
    ...
    CONTRACT_RECEIPT_REFS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE_CONTRACT_RECEIPT_REFS,
    LIVE_REPO_OR_TRANSCRIPT_KEYS,
    PASSTHROUGH_FIELDS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE_PASSTHROUGH_FIELDS,
    PLAYBOOK_DOCTRINE_EXPORT_KEYS,
    PROTECTED_SURFACE_EXCEPTION_KEYS,
    QUESTION_PROMPT as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE_QUESTION_PROMPT,
...
STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE_STATUS_ADMISSIBLE = (
    "stack_command_implementation_actual_owner_side_mutation_authority_class_choice_admissible"
)
NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE = (
    "no_stack_command_implementation_actual_owner_side_mutation_authority_class_choice"
)
QUESTION_PROMPT = (
    "Which actual owner-side mutation authority class, if any, may any later "
    "admitted surface choose for the already-admitted _stack command "
    ...
    "owner-side mutation authority-class-realization seam, and the "
    "now-bounded actual owner-side mutation surface-choice seam for `stack "
    "supervised-execution-home` without choosing it now, one actual "
    "owner-side mutation surface now, one actual `_stack` execution-home "
    "surface, one actual concrete command file, one actual downstream "
    "runtime-home value placement, one actual concrete `_stack` command "
    "implementation-surface choice, one actual `_stack` command "
    "implementation, one Playbook doctrine export path, deploy or "
    "publication work, or one protected-surface touch?"
)
CONTRACT_RECEIPT_REFS = (
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-AUTHORITY-CLASS-CHOICE-CONTRACT-FREEZE-PASS-749-2026-06-26.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-AUTHORITY-CLASS-CHOICE-OWNER-SURFACE-ADMISSION-PASS-750-2026-06-26.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-AUTHORITY-CLASS-CHOICE-SUPPORTING-LANE-ADMISSION-PASS-751-2026-06-26.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-AUTHORITY-CLASS-CHOICE-FIRST-IMPLEMENTATION-ADMISSION-PASS-752-2026-06-26.md",
)
...
PASSTHROUGH_FIELDS = (
    *STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE_PASSTHROUGH_FIELDS,
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE_QUESTION_PROMPT
        ...
        == list(STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE_CONTRACT_RECEIPT_REFS)
    )
...
def _attempt_reasons(bundle: Mapping[str, Any]) -> list[str]:
    ...
    checks = (
        ...
        (
            ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_KEYS,
            "actual_owner_side_mutation_authority_class_choice_attempted",
        ),
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("stack_command_implementation_actual_owner_side_mutation_surface_choice_status"))
        != STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_CHOICE_STATUS_ADMISSIBLE
    ):
        return ["stack_command_implementation_actual_owner_side_mutation_surface_choice_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_surface_choice_question"),
        candidate_path,
    ):
        reasons.append("stack_command_implementation_actual_owner_side_mutation_surface_choice_question_not_explicit")
    if _normalized_string_list(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_surface_choice_reasons")
    ):
        reasons.append("stack_command_implementation_actual_owner_side_mutation_surface_choice_reasons_present")
...
def evaluate_supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_authority_class_choice(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    result["stack_command_implementation_actual_owner_side_mutation_authority_class_choice_status"] = (
        STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE_STATUS_ADMISSIBLE
        if not reasons
        else NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_CHOICE
    )
    result["stack_command_implementation_actual_owner_side_mutation_authority_class_choice_question"] = (
        _question_card(candidate_path) if not reasons else None
    )
    result["stack_command_implementation_actual_owner_side_mutation_authority_class_choice_reasons"] = reasons
    return result
