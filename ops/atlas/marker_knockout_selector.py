...
PACKET_REGISTRY: dict[str, PacketDescriptor] = {
    ...
    "Sandbox Simulation Readiness": PacketDescriptor(
        packet="Sandbox Simulation Readiness local-only first validator-verdict activation gate contract freeze",
        basis_receipt_ref=(
            "docs/ops/"
            "SANDBOX-SIMULATION-READINESS-LOCAL-ONLY-FIRST-VALIDATOR-PAIR-COHERENCE-"
            "SEMANTICS-CONTRACT-FREEZE-2026-06-27.md"
        ),
        mode="root-owned docs-only validator-verdict-activation-gate follow-on",
        scope=(
            "freeze the exact gate that must open before any future linked validator-report may move beyond "
            "`result.status: not_run` into `match`, `mismatch`, or `blocked` without admitting validator execution, runner behavior, "
