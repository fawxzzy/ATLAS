...
from ops.atlas.supervised_execution_home_stack_command_implementation_worker_authority import (
    ...
    PASSTHROUGH_FIELDS as STACK_COMMAND_IMPLEMENTATION_WORKER_AUTHORITY_PASSTHROUGH_FIELDS,
...
STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_STATUS_ADMISSIBLE = (
    "stack_command_implementation_owner_repo_implementation_routing_admissible"
)
NO_STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING = (
    "no_stack_command_implementation_owner_repo_implementation_routing"
)
QUESTION_PROMPT = (
    "Which owner-repo implementation-routing contract, if any, may any later "
    "admitted surface define for the already-admitted _stack command "
    "implementation worker-authority seam for `stack supervised-execution-home` "
    "without defining one actual route now or widening into one actual concrete "
    "command file, one actual downstream runtime-home value placement, one "
    "actual concrete _stack command implementation surface choice, one actual "
    "_stack command implementation, one actual worker authority class, one "
    "actual owner-side mutation authority, one Playbook doctrine export path, "
    "or one protected-surface exception?"
)
CONTRACT_RECEIPT_REFS = (
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-OWNER-REPO-IMPLEMENTATION-ROUTING-CONTRACT-FREEZE-PASS-675-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-OWNER-REPO-IMPLEMENTATION-ROUTING-OWNER-SURFACE-ADMISSION-PASS-676-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-OWNER-REPO-IMPLEMENTATION-ROUTING-SUPPORTING-LANE-ADMISSION-PASS-677-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-OWNER-REPO-IMPLEMENTATION-ROUTING-FIRST-IMPLEMENTATION-ADMISSION-PASS-678-2026-06-25.md",
)
...
OWNER_REPO_IMPLEMENTATION_ROUTING_KEYS = {
    "implementation_route",
    "owner_repo_implementation_route",
    "owner_repo_implementation_routing",
    "owner_repo_route",
    "routing_card",
}
...
PASSTHROUGH_FIELDS = (
    *STACK_COMMAND_IMPLEMENTATION_WORKER_AUTHORITY_PASSTHROUGH_FIELDS,
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == STACK_COMMAND_IMPLEMENTATION_WORKER_AUTHORITY_QUESTION_PROMPT
        ...
        == list(STACK_COMMAND_IMPLEMENTATION_WORKER_AUTHORITY_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("stack_command_implementation_worker_authority_status"))
        != STACK_COMMAND_IMPLEMENTATION_WORKER_AUTHORITY_STATUS_ADMISSIBLE
    ):
        return ["stack_command_implementation_worker_authority_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("stack_command_implementation_worker_authority_question"),
        candidate_path,
    ):
        reasons.append("stack_command_implementation_worker_authority_question_not_explicit")
    if _normalized_string_list(bundle.get("stack_command_implementation_worker_authority_reasons")):
        reasons.append("stack_command_implementation_worker_authority_reasons_present")
...
def evaluate_supervised_execution_home_stack_command_implementation_owner_repo_implementation_routing(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    result["stack_command_implementation_owner_repo_implementation_routing_status"] = (
        STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING_STATUS_ADMISSIBLE
        if not reasons
        else NO_STACK_COMMAND_IMPLEMENTATION_OWNER_REPO_IMPLEMENTATION_ROUTING
    )
    result["stack_command_implementation_owner_repo_implementation_routing_question"] = (
        _question_card(candidate_path) if not reasons else None
    )
    result["stack_command_implementation_owner_repo_implementation_routing_reasons"] = reasons
    return result
