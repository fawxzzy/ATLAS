...
const fencePath = path.join(root, 'ops/atlas/invoke_supabase_mazer_master_cutover_data_fence_r001.ps1');
...
export function sealInvocation({ decisionRequestPath, aliasPath, authorizationPath, consumptionPath, outputPath, now = new Date() }) {
  ...
  for (const value of [s.private_source_sha256,s.manifest_sha256,s.auth_apply_sha256,s.postverify_sha256,s.host_sha256,s.credential_safe_launcher_sha256,s.materializer_sha256,s.classifier_sha256,s.fence_sha256,s.node_sha256,s.prior_rollback_receipt_sha256]) if (!digest(value)) throw new Error('DECISION_TUPLE_DIGEST');
  ...
  const decisionFencePath = resolveRelative(s.fence_path, path.dirname(fencePath), 'FENCE_PATH');
  ...
  for (const [file, expected] of [[sourcePath,s.private_source_sha256],[manifestPath,s.manifest_sha256],[hostPath,s.host_sha256],[materializerPath,s.materializer_sha256],[classifierPath,s.classifier_sha256],[fencePath,s.fence_sha256],[nodePath,s.node_sha256],[predecessorPath,s.prior_rollback_receipt_sha256]]) if (sha256(fs.readFileSync(file)) !== expected) throw new Error('SEALED_FILE_DIGEST');
  ...
  const issuedAt = formatO(now), envelope = {
    ...
    fence_path: fencePath, fence_sha256: s.fence_sha256,
