...
from ops.atlas.supervised_execution_home_stack_command_implementation_actual_owner_side_mutation import (
    ...
    PASSTHROUGH_FIELDS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_PASSTHROUGH_FIELDS,
...
STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_STATUS_ADMISSIBLE = (
    "stack_command_implementation_actual_owner_side_mutation_authority_admissible"
)
NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY = (
    "no_stack_command_implementation_actual_owner_side_mutation_authority"
)
QUESTION_PROMPT = (
    "Which actual owner-side mutation authority contract, if any, may any later "
    "admitted surface define for the already-admitted _stack command "
    "implementation actual owner-side mutation seam for `stack "
    "supervised-execution-home` without defining one actual owner-side mutation "
    "surface, one actual owner-side mutation authority class now, one actual "
    "`_stack` execution-home surface, one actual concrete command file, one "
    "actual downstream runtime-home value placement, one actual concrete "
...
PASSTHROUGH_FIELDS = (
    *STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_PASSTHROUGH_FIELDS,
    "stack_command_implementation_actual_owner_side_mutation_status",
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_QUESTION_PROMPT
        ...
        == list(STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("stack_command_implementation_actual_owner_side_mutation_status"))
        != STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_STATUS_ADMISSIBLE
    ):
        return ["stack_command_implementation_actual_owner_side_mutation_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_question"),
        candidate_path,
    ):
        reasons.append(
    ...
    if _normalized_string_list(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_reasons")
    ):
        reasons.append("stack_command_implementation_actual_owner_side_mutation_reasons_present")
...
def evaluate_supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_authority(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    result["stack_command_implementation_actual_owner_side_mutation_authority_status"] = (
        STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_STATUS_ADMISSIBLE
        if not reasons
        else NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY
    )
    result["stack_command_implementation_actual_owner_side_mutation_authority_question"] = (
        _question_card(candidate_path) if not reasons else None
    )
    result["stack_command_implementation_actual_owner_side_mutation_authority_reasons"] = reasons
    return result
