...
export const PRODUCER_CONTRACT = Object.freeze({
  ...
  evidence: Object.freeze({
    currentPreimage: Object.freeze({
      relativePath: 'runtime/atlas/continuity/mazer-master-r017-ai-receipt-rebound-evidence-20260826.json',
      sha256: R017_CONTRACT.currentPreimageSha256,
      result: 'PASS_EXACT_AI_AND_RECEIPT_DELTA_READY_FOR_R017_REPLAY'
    }),
...
export function verifyEvidence(atlasRoot) {
  ...
  const actual = {
    legacy: { auth_users: 19, auth_identities: 19, profiles: 13, player: 16, ai: 16, receipts: 1877 },
...
end $r017$;`, ['data_api','rls','acl','auth.users','114','13','16','1886']);
...
do $r017_auth_precheck$ begin
 if exists(select 1 from jsonb_array_elements(${jsonLiteral(existingBindings)}) e where not exists(select 1 from auth.users u where u.id=(e->'user'->>'id')::uuid and lower(u.email)=e->>'normalized_email' and to_jsonb(u)=to_jsonb(jsonb_populate_record(null::auth.users,e->'user')))) then raise exception 'R017_EXISTING_AUTH_USER_DIGEST_DRIFT'; end if;
 if exists(select 1 from jsonb_array_elements(${jsonLiteral(existingBindings)}) e where not exists(select 1 from auth.identities i where i.id=(e->'identity'->>'id')::uuid and i.user_id=(e->'user'->>'id')::uuid and i.provider='email' and i.provider_id=e->'user'->>'id' and to_jsonb(i)=to_jsonb(jsonb_populate_record(null::auth.identities,e->'identity')))) then raise exception 'R017_EXISTING_AUTH_IDENTITY_DIGEST_DRIFT'; end if;
...
end $username_key$;`, ['whole_row_override','8/7/36/D','39/108/161/S','pgp_sym_encrypt','player_reset_disposition','vault.create_secret','rollback_bound_username_key']);
...
do $r017$ begin
 ...
 if exists(select 1 from jsonb_array_elements(${jsonLiteral(existingBindings)}) e where not exists(select 1 from auth.users u where u.id=(e->'user'->>'id')::uuid and lower(u.email)=e->>'normalized_email' and to_jsonb(u)=to_jsonb(jsonb_populate_record(null::auth.users,e->'user')))) then raise exception 'R017_BOUND_AUTH_USERS_FULL_DIGEST_DRIFT'; end if;
 if exists(select 1 from jsonb_array_elements(${jsonLiteral(existingBindings)}) e where not exists(select 1 from auth.identities i where i.id=(e->'identity'->>'id')::uuid and i.user_id=(e->'user'->>'id')::uuid and i.provider='email' and i.provider_id=e->'user'->>'id' and to_jsonb(i)=to_jsonb(jsonb_populate_record(null::auth.identities,e->'identity')))) then raise exception 'R017_BOUND_AUTH_IDENTITIES_FULL_DIGEST_DRIFT'; end if;
...
end $r017$;`, ['data_api','rls','acl','117','19','13','16','1886','receipt_conservation']);
...
export function producePrivateSource({ legacy, master, legacyAcl, masterAcl, quarantineKey, qaPassword }) {
  ...
  const resetLegacy = legacy.ai.find((row) => String(row.level) === '8' && String(row.completed_cycles) === '7' && Number(row.target_complexity) === 36 && row.rank === 'D');
  ...
  if (legacyReceiptCount !== 1715 || masterReceiptCount !== 1239) throw new Error('RESET_RECEIPT_DENOMINATOR_DRIFT');
  ...
  const reset_era_ai = { legacy_user_id: edge.legacy_user_id, master_user_id: edge.master_user_id, canonical_projection: '8/7/36/D', quarantined_projection: '39/108/161/S', legacy_receipts: legacyReceiptCount, master_receipts: masterReceiptCount, legacy_timestamps_newer: true, override_mode: 'EXACT_WHOLE_ROW', quarantine_encryption: 'PGP_SYM_ENCRYPT_AES256', canonical_row_digest: digest(sourceAiEnvelope), quarantined_row_digest: digest(targetAiEnvelope), quarantined_row: resetMaster };
