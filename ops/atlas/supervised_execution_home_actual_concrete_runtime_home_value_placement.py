...
from ops.atlas.supervised_execution_home_actual_concrete_runtime_home_value import (
    ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_STATUS_ADMISSIBLE,
    CONTRACT_RECEIPT_REFS as ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_CONTRACT_RECEIPT_REFS,
    QUESTION_PROMPT as ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_QUESTION_PROMPT,
)

ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_PLACEMENT_STATUS_ADMISSIBLE = (
...
QUESTION_PROMPT = (
    "Where, if anywhere, may any later admitted surface place the actual concrete "
    "runtime-home value allowed by the already-admitted actual-concrete-runtime-home-"
    "value seam for `stack supervised-execution-home` without placing it now or "
    "widening into concrete `_stack` command-home choice, concrete command-file "
    "choice, concrete implementation-surface choice, `_stack` implementation, "
    "worker authority, owner-repo edits, actual owner-side mutation authority, "
    "Playbook doctrine export, or protected-surface exceptions?"
)
...
ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_PLACEMENT_KEYS = {
    "actual_concrete_runtime_home_value_placement",
    ...
    "actual_runtime_home_value_placement",
    "placement_path",
    "placement_surface",
    "placement_target",
    "runtime_home_value_location",
    "runtime_home_value_placement",
    "runtime_value_placement",
}
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_QUESTION_PROMPT
        ...
        == list(ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(bundle.get("actual_concrete_runtime_home_value_status"))
        != ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_STATUS_ADMISSIBLE
    ):
        return ["actual_concrete_runtime_home_value_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("actual_concrete_runtime_home_value_question"), candidate_path
    ):
        reasons.append("actual_concrete_runtime_home_value_question_not_explicit")
    if _normalized_string_list(bundle.get("actual_concrete_runtime_home_value_reasons")):
        reasons.append("actual_concrete_runtime_home_value_reasons_present")
...
def evaluate_supervised_execution_home_actual_concrete_runtime_home_value_placement(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    actual_concrete_runtime_home_choice_question = bundle.get(
        "actual_concrete_runtime_home_choice_question"
    )
    actual_concrete_runtime_home_value_choice_question = bundle.get(
        "actual_concrete_runtime_home_value_choice_question"
    )
    actual_concrete_runtime_home_value_question = bundle.get(
    ...
    return {
        ...
        "actual_concrete_runtime_home_choice_status": _normalized_text(
            bundle.get("actual_concrete_runtime_home_choice_status")
        ),
        "actual_concrete_runtime_home_choice_question": (
            dict(actual_concrete_runtime_home_choice_question)
            if isinstance(actual_concrete_runtime_home_choice_question, Mapping)
            else None
        ),
        "actual_concrete_runtime_home_choice_reasons": _normalized_string_list(
            bundle.get("actual_concrete_runtime_home_choice_reasons")
        ),
        "actual_concrete_runtime_home_value_choice_status": _normalized_text(
            bundle.get("actual_concrete_runtime_home_value_choice_status")
        ),
        "actual_concrete_runtime_home_value_choice_question": (
            dict(actual_concrete_runtime_home_value_choice_question)
            if isinstance(actual_concrete_runtime_home_value_choice_question, Mapping)
            else None
        ),
        "actual_concrete_runtime_home_value_choice_reasons": _normalized_string_list(
            bundle.get("actual_concrete_runtime_home_value_choice_reasons")
        ),
        "actual_concrete_runtime_home_value_status": _normalized_text(
            bundle.get("actual_concrete_runtime_home_value_status")
        ),
        "actual_concrete_runtime_home_value_question": (
            dict(actual_concrete_runtime_home_value_question)
            if isinstance(actual_concrete_runtime_home_value_question, Mapping)
            else None
        ),
        "actual_concrete_runtime_home_value_reasons": _normalized_string_list(
            bundle.get("actual_concrete_runtime_home_value_reasons")
        ),
        "actual_concrete_runtime_home_value_placement_status": (
            ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_PLACEMENT_STATUS_ADMISSIBLE
            if not reasons
            else NO_ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_PLACEMENT
        ),
        "actual_concrete_runtime_home_value_placement_question": (
            _question_card(candidate_path) if not reasons else None
        ),
        "actual_concrete_runtime_home_value_placement_reasons": reasons,
    }
