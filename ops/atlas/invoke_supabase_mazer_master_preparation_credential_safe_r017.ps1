...
function Read-Invocation([string]$Path, [string]$ExpectedSha) {
  ...
  $expectedKeys = @('schema','packet','predecessor_correlation_id','predecessor_state_path','predecessor_state_sha256','execution_correlation_id','private_source_path','private_source_sha256','private_manifest_path','private_manifest_sha256','successor_state_path','host_path','host_sha256','issued_at','expires_at')
  ...
  foreach ($key in @($expectedKeys | Where-Object { $_ -notin @('issued_at','expires_at') })) { if ($value.$key -isnot [string]) { throw 'INVOCATION_VALUE_TYPES' } }
  if ([string]$value.schema -cne 'atlas.supabase.mazer-master-preparation-launcher-invocation.r017.v1' -or [string]$value.packet -cne $Packet) { throw 'INVOCATION_SCHEMA' }
  ...
  if ($hostPath -cne [IO.Path]::GetFullPath($HostScript)) { throw 'HOST_PATH_BINDING' }
  foreach ($file in @($source,$manifest,$predecessor,$hostPath)) { if (-not (Test-Path -LiteralPath $file -PathType Leaf)) { throw 'SEALED_FILE_MISSING' } }
  Assert-NoReparse $source $PacketRoot; Assert-NoReparse $manifest $PacketRoot; Assert-NoReparse $predecessor $Runtime; Assert-NoReparse $hostPath $PSScriptRoot
  if ((Get-Sha256 $source) -cne $SourceSha -or (Get-Sha256 $manifest) -cne $ManifestSha -or (Get-Sha256 $predecessor) -cne $PredecessorStateSha -or (Get-Sha256 $hostPath) -cne $HostSha) { throw 'SEALED_FILE_DIGEST_DRIFT' }
  ...
  $issuedMatch=[regex]::Match($text,'"issued_at"\s*:\s*"([^"]+)"');$expiresMatch=[regex]::Match($text,'"expires_at"\s*:\s*"([^"]+)"')
  if(-not$issuedMatch.Success-or-not$expiresMatch.Success){throw 'INVOCATION_TIMESTAMP'}
  try { $issued=[DateTimeOffset]::ParseExact($issuedMatch.Groups[1].Value,'o',[Globalization.CultureInfo]::InvariantCulture);$expires=[DateTimeOffset]::ParseExact($expiresMatch.Groups[1].Value,'o',[Globalization.CultureInfo]::InvariantCulture) } catch { throw 'INVOCATION_TIMESTAMP' }
  $now=[DateTimeOffset]::UtcNow
  if ($issued -gt $now.AddSeconds(5) -or $issued -lt $now.AddMinutes(-10) -or $expires -le $now -or $expires -gt $issued.AddHours(24)) { throw 'INVOCATION_STALE' }
