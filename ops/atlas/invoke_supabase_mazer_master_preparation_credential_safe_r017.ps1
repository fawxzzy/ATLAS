...
$MaterializerScript = Join-Path $PSScriptRoot 'materialize_supabase_mazer_master_preparation_r017.mjs'
$ClassifierScript = Join-Path $PSScriptRoot 'classify_supabase_mazer_master_cutover_data_fence_r001.mjs'
$FenceScript = Join-Path $PSScriptRoot 'invoke_supabase_mazer_master_cutover_data_fence_r001.ps1'
...
function Assert-NoReparseAbsolute([string]$Path) {
  $candidate = [IO.Path]::GetFullPath($Path)
  if (-not [IO.Path]::IsPathRooted($candidate) -or -not (Test-Path -LiteralPath $candidate -PathType Leaf)) { throw 'NODE_PATH' }
  $root = [IO.Path]::GetPathRoot($candidate); $cursor = $root
  foreach ($part in $candidate.Substring($root.Length).Split([IO.Path]::DirectorySeparatorChar,[StringSplitOptions]::RemoveEmptyEntries)) {
    ...
    if ((Get-Item -LiteralPath $cursor -Force).Attributes -band [IO.FileAttributes]::ReparsePoint) { throw 'NODE_REPARSE' }
  }
  return $candidate
}

function Get-CanonicalNodeCommandPath([string]$VerifiedNodePath,[string]$ExpectedNodeSha) {
  $verified=Assert-NoReparseAbsolute $VerifiedNodePath
  if($ExpectedNodeSha-cnotmatch'^[a-f0-9]{64}$'){throw 'NODE_RUNTIME_DIGEST'}
  $stream=New-Object IO.FileStream($verified,[IO.FileMode]::Open,[IO.FileAccess]::Read,[IO.FileShare]::Read)
  try{
    $hasher=[Security.Cryptography.SHA256]::Create();try{$actual=([BitConverter]::ToString($hasher.ComputeHash($stream))).Replace('-','').ToLowerInvariant()}finally{$hasher.Dispose()}
    if($actual-cne$ExpectedNodeSha){throw 'NODE_RUNTIME_DIGEST'}
    $command=(Get-Command node -ErrorAction Stop).Source
    $probe=Invoke-Child $verified @('-p',"require('node:fs').realpathSync.native(process.argv[1])",$command) @{SUPABASE_ACCESS_TOKEN='';ATLAS_MAZER_LEGACY_DATABASE_URL='';ATLAS_MAZER_MASTER_DATABASE_URL='';ATLAS_MAZER_R017_QUARANTINE_KEY='';ATLAS_MAZER_R017_QA_PASSWORD=''} 30000
    if($probe.ExitCode-ne0-or-not[string]::IsNullOrEmpty($probe.Stderr)-or[string]::IsNullOrWhiteSpace($probe.Stdout)){throw 'NODE_RUNTIME_PATH'}
    return Assert-NoReparseAbsolute ([string]$probe.Stdout.Trim())
  }finally{$stream.Dispose()}
}
...
function Read-Invocation([string]$Path, [string]$ExpectedSha) {
  ...
  $expectedKeys = @('schema','packet','decision_request_path','decision_request_sha256','approval_alias_path','approval_alias_sha256','approval_authorization_path','approval_authorization_sha256','approval_consumption_path','approval_consumption_sha256','approval_expires_at','predecessor_state_path','predecessor_state_sha256','execution_correlation_id','private_source_path','private_source_sha256','private_manifest_path','private_manifest_sha256','successor_state_path','host_path','host_sha256','launcher_path','launcher_sha256','materializer_path','materializer_sha256','classifier_path','classifier_sha256','fence_path','fence_sha256','node_path','node_sha256','terminal_final_identity_edges','terminal_profiles','terminal_player','terminal_ai','terminal_receipts','not_before','issued_at','expires_at')
  ...
  foreach($key in @('decision_request_sha256','approval_alias_sha256','approval_authorization_sha256','approval_consumption_sha256','predecessor_state_sha256','private_source_sha256','private_manifest_sha256','host_sha256','launcher_sha256','materializer_sha256','classifier_sha256','fence_sha256','node_sha256')){if([string]$value.$key-cnotmatch'^[a-f0-9]{64}$'){throw 'SEALED_HASH_BINDING'}}
  ...
  $materializerPath = Assert-Under ([string]$value.materializer_path) $PSScriptRoot
  $classifierPath = Assert-Under ([string]$value.classifier_path) $PSScriptRoot
  $fencePath = Assert-Under ([string]$value.fence_path) $PSScriptRoot
  $nodePath = Assert-NoReparseAbsolute ([string]$value.node_path)
  ...
  $runtimeNodePath=Get-CanonicalNodeCommandPath $nodePath ([string]$value.node_sha256)
  if ($hostPath -cne [IO.Path]::GetFullPath($HostScript) -or $launcherPath -cne [IO.Path]::GetFullPath($PSCommandPath) -or $materializerPath -cne [IO.Path]::GetFullPath($MaterializerScript) -or $classifierPath -cne [IO.Path]::GetFullPath($ClassifierScript) -or $fencePath -cne [IO.Path]::GetFullPath($FenceScript) -or $nodePath -cne $runtimeNodePath) { throw 'HOST_PATH_BINDING' }
  foreach ($file in @($source,$manifest,$predecessor,$hostPath,$launcherPath,$materializerPath,$classifierPath,$fencePath,$nodePath)) { if (-not (Test-Path -LiteralPath $file -PathType Leaf)) { throw 'SEALED_FILE_MISSING' } }
  Assert-NoReparse $source $PacketRoot; Assert-NoReparse $manifest $PacketRoot; Assert-NoReparse $predecessor $Runtime; Assert-NoReparse $hostPath $PSScriptRoot; Assert-NoReparse $launcherPath $PSScriptRoot; Assert-NoReparse $materializerPath $PSScriptRoot; Assert-NoReparse $classifierPath $PSScriptRoot; Assert-NoReparse $fencePath $PSScriptRoot
  if ((Get-Sha256 $source) -cne [string]$value.private_source_sha256 -or (Get-Sha256 $manifest) -cne [string]$value.private_manifest_sha256 -or (Get-Sha256 $predecessor) -cne [string]$value.predecessor_state_sha256 -or (Get-Sha256 $hostPath) -cne [string]$value.host_sha256 -or (Get-Sha256 $launcherPath) -cne [string]$value.launcher_sha256 -or (Get-Sha256 $materializerPath) -cne [string]$value.materializer_sha256 -or (Get-Sha256 $classifierPath) -cne [string]$value.classifier_sha256 -or (Get-Sha256 $fencePath) -cne [string]$value.fence_sha256 -or (Get-Sha256 $nodePath) -cne [string]$value.node_sha256) { throw 'SEALED_FILE_DIGEST_DRIFT' }
  try { $prior = Get-Content -LiteralPath $predecessor -Raw | ConvertFrom-Json } catch { throw 'PREDECESSOR_STATE_JSON' }
  $priorFenceRestored = [string]$prior.fence_phase -ceq 'ROLLED_BACK' -or
    ([string]$prior.fence_phase -ceq 'PREPARATION_COMPLETE' -and [string]$prior.postrestore_proof -ceq 'HOST_EXACT_ROLLBACK_COMPLETED_AND_LEGACY_RELEASED')
  if ([string]$prior.schema -cne 'atlas.supabase.mazer-master-preparation-terminal-receipt.r017.v1' -or [string]$prior.packet -cne $Packet -or [string]$prior.result -cne 'HOLD_MAZER_MASTER_PREPARATION_R017' -or [string]$prior.rollback_disposition -cne 'EXACT_ROLLBACK_COMPLETED' -or [string]$prior.execution_correlation_id -cnotmatch '^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$' -or [string]$prior.host_phase -cne 'ROLLED_BACK' -or -not $priorFenceRestored -or [bool]$prior.watchdog_running -or [int]$prior.provider_writes -ne 0 -or [int]$prior.environment_changes -ne 0 -or [int]$prior.deployments -ne 0 -or [int]$prior.production_changes -ne 0 -or [bool]$prior.raw_records_emitted -or [bool]$prior.pii_emitted -or [bool]$prior.secrets_emitted -or [string]$prior.replay_disposition -cne 'CONSUMED_NON_REPLAYABLE') { throw 'PREDECESSOR_EFFECT_STATE' }
  ...
  $sealedKeys=@('execution_correlation_id','private_source_path','private_source_sha256','manifest_path','manifest_sha256','auth_apply_sha256','postverify_sha256','host_path','host_sha256','credential_safe_launcher_path','credential_safe_launcher_sha256','materializer_path','materializer_sha256','classifier_path','classifier_sha256','fence_path','fence_sha256','node_path','node_sha256','packet_merge_commit','jit_invocation_merge_commit','independent_review_checkpoint','prior_rollback_state_path','prior_rollback_receipt_sha256')
  ...
  foreach($key in @('private_source_sha256','manifest_sha256','auth_apply_sha256','postverify_sha256','host_sha256','credential_safe_launcher_sha256','materializer_sha256','classifier_sha256','fence_sha256','node_sha256','prior_rollback_receipt_sha256')){if([string]$d.sealed_inputs.$key-cnotmatch'^[a-f0-9]{64}$'){throw 'DECISION_TUPLE_DIGEST'}}
  ...
  if ([string]$d.sealed_inputs.execution_correlation_id-cne[string]$value.execution_correlation_id-or(Resolve-DecisionRelative ([string]$d.sealed_inputs.private_source_path) 'DECISION_SOURCE_PATH')-cne$source-or[string]$d.sealed_inputs.private_source_sha256-cne[string]$value.private_source_sha256-or(Resolve-DecisionRelative ([string]$d.sealed_inputs.manifest_path) 'DECISION_MANIFEST_PATH')-cne$manifest-or[string]$d.sealed_inputs.manifest_sha256-cne[string]$value.private_manifest_sha256-or(Resolve-DecisionRelative ([string]$d.sealed_inputs.host_path) 'DECISION_HOST_PATH')-cne$hostPath-or[string]$d.sealed_inputs.host_sha256-cne[string]$value.host_sha256-or(Resolve-DecisionRelative ([string]$d.sealed_inputs.credential_safe_launcher_path) 'DECISION_LAUNCHER_PATH')-cne$launcherPath-or[string]$d.sealed_inputs.credential_safe_launcher_sha256-cne[string]$value.launcher_sha256-or(Resolve-DecisionRelative ([string]$d.sealed_inputs.materializer_path) 'DECISION_MATERIALIZER_PATH')-cne$materializerPath-or[string]$d.sealed_inputs.materializer_sha256-cne[string]$value.materializer_sha256-or(Resolve-DecisionRelative ([string]$d.sealed_inputs.classifier_path) 'DECISION_CLASSIFIER_PATH')-cne$classifierPath-or[string]$d.sealed_inputs.classifier_sha256-cne[string]$value.classifier_sha256-or(Resolve-DecisionRelative ([string]$d.sealed_inputs.fence_path) 'DECISION_FENCE_PATH')-cne$fencePath-or[string]$d.sealed_inputs.fence_sha256-cne[string]$value.fence_sha256-or[IO.Path]::GetFullPath([string]$d.sealed_inputs.node_path)-cne$nodePath-or[string]$d.sealed_inputs.node_sha256-cne[string]$value.node_sha256-or(Resolve-DecisionRelative ([string]$d.sealed_inputs.prior_rollback_state_path) 'DECISION_PREDECESSOR_PATH')-cne$predecessor-or[string]$d.sealed_inputs.prior_rollback_receipt_sha256-cne[string]$value.predecessor_state_sha256) { throw 'DECISION_EXECUTION_BINDING' }
  ...
  return [pscustomobject]@{ Source=$source; SourceSha=[string]$value.private_source_sha256; HostSha=[string]$value.host_sha256; Materializer=$materializerPath; MaterializerSha=[string]$value.materializer_sha256; Classifier=$classifierPath; ClassifierSha=[string]$value.classifier_sha256; Fence=$fencePath; FenceSha=[string]$value.fence_sha256; Node=$nodePath; NodeSha=[string]$value.node_sha256; State=$successor; Correlation=[string]$value.execution_correlation_id; Counts=[pscustomobject]@{final_identity_edges=[int]$value.terminal_final_identity_edges;profiles=[int]$value.terminal_profiles;player=[int]$value.terminal_player;ai=[int]$value.terminal_ai;receipts=[int]$value.terminal_receipts} }
}
...
try {
  ...
  $psqlShim=Join-Path $hostRoot 'tmp\r017-psql-shim-reviewed2';$psqlBin='C:\Program Files\PostgreSQL\17\bin'
  if((Get-Sha256 (Join-Path $psqlShim 'psql.exe'))-cne$PsqlShimSha){throw 'SHIM_DIGEST_DRIFT'}
  ...
  $environment=@{SUPABASE_ACCESS_TOKEN=$token;ATLAS_MAZER_LEGACY_DATABASE_URL="postgresql://postgres:$([Uri]::EscapeDataString($legacyText))@db.${LegacyRef}.supabase.co:5432/postgres?sslmode=require";ATLAS_MAZER_MASTER_DATABASE_URL="postgresql://postgres.${MasterRef}:$([Uri]::EscapeDataString($masterText))@aws-0-ca-central-1.pooler.supabase.com:5432/postgres?sslmode=require";ATLAS_R017_VERIFIED_MATERIALIZER_PATH=$bound.Materializer;ATLAS_R017_VERIFIED_MATERIALIZER_SHA256=$bound.MaterializerSha;ATLAS_R017_VERIFIED_CLASSIFIER_PATH=$bound.Classifier;ATLAS_R017_VERIFIED_CLASSIFIER_SHA256=$bound.ClassifierSha;ATLAS_R017_VERIFIED_FENCE_PATH=$bound.Fence;ATLAS_R017_VERIFIED_FENCE_SHA256=$bound.FenceSha;ATLAS_R017_VERIFIED_NODE_PATH=$bound.Node;ATLAS_R017_VERIFIED_NODE_SHA256=$bound.NodeSha;PATH=$psqlShim+[IO.Path]::PathSeparator+$psqlBin+[IO.Path]::PathSeparator+$env:PATH}
