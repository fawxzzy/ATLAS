...
function Read-Invocation([string]$Path, [string]$ExpectedSha) {
  ...
  $expectedKeys = @('schema','packet','decision_request_path','decision_request_sha256','approval_alias_path','approval_alias_sha256','approval_authorization_path','approval_authorization_sha256','approval_consumption_path','approval_consumption_sha256','approval_expires_at','predecessor_state_path','predecessor_state_sha256','execution_correlation_id','private_source_path','private_source_sha256','private_manifest_path','private_manifest_sha256','successor_state_path','host_path','host_sha256','launcher_path','launcher_sha256','materializer_path','materializer_sha256','classifier_path','classifier_sha256','node_path','node_sha256','terminal_final_identity_edges','terminal_profiles','terminal_player','terminal_ai','terminal_receipts','not_before','issued_at','expires_at')
  ...
  foreach($key in @('decision_request_sha256','approval_alias_sha256','approval_authorization_sha256','approval_consumption_sha256','predecessor_state_sha256','private_source_sha256','private_manifest_sha256','host_sha256','launcher_sha256','materializer_sha256','classifier_sha256','node_sha256')){if([string]$value.$key-cnotmatch'^[a-f0-9]{64}$'){throw 'SEALED_HASH_BINDING'}}
  ...
  if ($hostPath -cne [IO.Path]::GetFullPath($HostScript) -or $launcherPath -cne [IO.Path]::GetFullPath($PSCommandPath) -or $materializerPath -cne [IO.Path]::GetFullPath($MaterializerScript) -or $classifierPath -cne [IO.Path]::GetFullPath($ClassifierScript) -or $nodePath -cne $runtimeNodePath) { throw 'HOST_PATH_BINDING' }
  foreach ($file in @($source,$manifest,$predecessor,$hostPath,$launcherPath,$materializerPath,$classifierPath,$nodePath)) { if (-not (Test-Path -LiteralPath $file -PathType Leaf)) { throw 'SEALED_FILE_MISSING' } }
  Assert-NoReparse $source $PacketRoot; Assert-NoReparse $manifest $PacketRoot; Assert-NoReparse $predecessor $Runtime; Assert-NoReparse $hostPath $PSScriptRoot; Assert-NoReparse $launcherPath $PSScriptRoot; Assert-NoReparse $materializerPath $PSScriptRoot; Assert-NoReparse $classifierPath $PSScriptRoot
  if ((Get-Sha256 $source) -cne [string]$value.private_source_sha256 -or (Get-Sha256 $manifest) -cne [string]$value.private_manifest_sha256 -or (Get-Sha256 $predecessor) -cne [string]$value.predecessor_state_sha256 -or (Get-Sha256 $hostPath) -cne [string]$value.host_sha256 -or (Get-Sha256 $launcherPath) -cne [string]$value.launcher_sha256 -or (Get-Sha256 $materializerPath) -cne [string]$value.materializer_sha256 -or (Get-Sha256 $classifierPath) -cne [string]$value.classifier_sha256 -or (Get-Sha256 $nodePath) -cne [string]$value.node_sha256) { throw 'SEALED_FILE_DIGEST_DRIFT' }
  ...
  foreach($key in @('private_source_sha256','manifest_sha256','auth_apply_sha256','postverify_sha256','host_sha256','credential_safe_launcher_sha256','materializer_sha256','classifier_sha256','node_sha256','prior_rollback_receipt_sha256')){if([string]$d.sealed_inputs.$key-cnotmatch'^[a-f0-9]{64}$'){throw 'DECISION_TUPLE_DIGEST'}}
  ...
  return [pscustomobject]@{ Source=$source; SourceSha=[string]$value.private_source_sha256; HostSha=[string]$value.host_sha256; Materializer=$materializerPath; MaterializerSha=[string]$value.materializer_sha256; Classifier=$classifierPath; ClassifierSha=[string]$value.classifier_sha256; Node=$nodePath; NodeSha=[string]$value.node_sha256; State=$successor; Correlation=[string]$value.execution_correlation_id; Counts=[pscustomobject]@{final_identity_edges=[int]$value.terminal_final_identity_edges;profiles=[int]$value.terminal_profiles;player=[int]$value.terminal_player;ai=[int]$value.terminal_ai;receipts=[int]$value.terminal_receipts} }
}
