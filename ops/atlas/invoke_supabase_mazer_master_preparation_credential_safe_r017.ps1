...
function Read-Invocation([string]$Path, [string]$ExpectedSha) {
  ...
  if ([string]$a.allowed_effect.effect_class-cne$EffectClass-or[string]$a.allowed_effect.target-cne$EffectTarget-or[int]$a.allowed_effect.max_effect_count-ne$MaxEffectCount-or[string]$a.decision_request.sha256-cne$decision.Sha256) { throw 'ALIAS_BINDING' }
  if ([string]$z.schema-cne'atlas.scoped-approval-authorization.v1'-or[string]$z.packet-cne$Packet-or[string]$z.status-cne'AUTHORIZED_SINGLE_USE'-or-not[bool]$z.single_use-or-not[bool]$z.execution_authority-or[string]$z.originating_task_id-cne$OriginatingTaskId-or[string]$z.approval_code-cne[string]$a.approval_code-or[string]$z.alias.sha256-cne$alias.Sha256-or[string]$z.decision_request.sha256-cne$decision.Sha256-or[string]$z.intent_digest-cne[string]$a.intent_digest) { throw 'AUTHORIZATION_BINDING' }
  if ([string]$c.schema-cne'atlas.scoped-approval-consumption.v1'-or[string]$c.packet-cne$Packet-or[string]$c.status-cne'CONSUMED'-or[bool]$c.reusable-or[int]$c.max_effect_count-ne$MaxEffectCount-or[string]$c.authorization_sha256-cne$authorization.Sha256-or[string]$c.intent_digest-cne[string]$a.intent_digest-or[string]$c.execution_correlation_id-cne[string]$value.execution_correlation_id) { throw 'CONSUMPTION_BINDING' }
