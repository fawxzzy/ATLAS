...
CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE_STATUS_ADMISSIBLE = (
    "concrete_stack_command_implementation_surface_choice_admissible"
)
NO_CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE = (
    "no_concrete_stack_command_implementation_surface_choice"
)
QUESTION_PROMPT = (
    "Which concrete _stack command implementation surface, if any, may carry the "
    "already-admitted actual concrete command-file downstream runtime-home value "
    "placement seam for `stack supervised-execution-home` without choosing it now or "
    "widening into one actual concrete command file, one actual downstream "
    "runtime-home value placement, one _stack command implementation, one worker "
    "authority class, one owner-repo edit authority, one actual owner-side mutation "
    "authority, one Playbook doctrine export path, or one protected-surface "
    "exception?"
)
CONTRACT_RECEIPT_REFS = (
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-CONCRETE-STACK-COMMAND-IMPLEMENTATION-SURFACE-CHOICE-CONTRACT-FREEZE-PASS-645-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-CONCRETE-STACK-COMMAND-IMPLEMENTATION-SURFACE-CHOICE-OWNER-SURFACE-ADMISSION-PASS-646-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-CONCRETE-STACK-COMMAND-IMPLEMENTATION-SURFACE-CHOICE-SUPPORTING-LANE-ADMISSION-PASS-647-2026-06-25.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-CONCRETE-STACK-COMMAND-IMPLEMENTATION-SURFACE-CHOICE-FIRST-IMPLEMENTATION-ADMISSION-PASS-648-2026-06-25.md",
)
...
CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE_KEYS = {
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_SELECTION_QUESTION_PROMPT
        ...
        == list(CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_SELECTION_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        ...
        return ["concrete_stack_command_implementation_surface_selection_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("concrete_stack_command_implementation_surface_selection_question"),
        candidate_path,
    ):
        reasons.append(
...
def evaluate_supervised_execution_home_concrete_stack_command_implementation_surface_choice(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    return {
        ...
        "concrete_stack_command_implementation_surface_choice_status": (
            CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE_STATUS_ADMISSIBLE
            if not reasons
            else NO_CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CHOICE
        ),
        "concrete_stack_command_implementation_surface_choice_question": (
            _question_card(candidate_path) if not reasons else None
        ),
        "concrete_stack_command_implementation_surface_choice_reasons": reasons,
    }
