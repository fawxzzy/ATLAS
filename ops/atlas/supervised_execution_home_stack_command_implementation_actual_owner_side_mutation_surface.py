...
from ops.atlas.supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_authority import (
    CONTRACT_RECEIPT_REFS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CONTRACT_RECEIPT_REFS,
    PASSTHROUGH_FIELDS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_PASSTHROUGH_FIELDS,
    QUESTION_PROMPT as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_QUESTION_PROMPT,
...
STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_STATUS_ADMISSIBLE = (
    "stack_command_implementation_actual_owner_side_mutation_surface_admissible"
)
NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE = (
    "no_stack_command_implementation_actual_owner_side_mutation_surface"
)
QUESTION_PROMPT = (
    "Which actual owner-side mutation surface contract, if any, may any later "
    "admitted surface define for the already-admitted _stack command "
    "implementation actual owner-side mutation authority seam for `stack "
    "supervised-execution-home` without defining one actual owner-side mutation "
    "surface now, one actual owner-side mutation authority class now, one "
    "actual `_stack` execution-home surface, one actual concrete command file, "
    "one actual downstream runtime-home value placement, one actual concrete "
    "`_stack` command implementation-surface choice, one actual `_stack` "
    "command implementation, one Playbook doctrine export path, deploy or "
    "publication work, or one protected-surface touch?"
)
CONTRACT_RECEIPT_REFS = (
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-SURFACE-CONTRACT-FREEZE-PASS-712-2026-06-26.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-SURFACE-OWNER-SURFACE-ADMISSION-PASS-713-2026-06-26.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-SURFACE-SUPPORTING-LANE-ADMISSION-PASS-714-2026-06-26.md",
    "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-SURFACE-FIRST-IMPLEMENTATION-ADMISSION-PASS-715-2026-06-26.md",
)
...
ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CLASS_KEYS = {
    ...
    "actual_owner_side_mutation_authority_role",
    "mutation_execution_authority",
    "mutation_execution_authority_class",
...
PASSTHROUGH_FIELDS = (
    *STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_PASSTHROUGH_FIELDS,
...
def _question_card_is_explicit(value: Any, candidate_path: str) -> bool:
    ...
    return (
        ...
        == STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_QUESTION_PROMPT
        ...
        == list(STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CONTRACT_RECEIPT_REFS)
    )
...
def _selection_reasons(
    ...
    if (
        _normalized_text(
            bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_status")
        )
        != STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_STATUS_ADMISSIBLE
    ):
        return ["stack_command_implementation_actual_owner_side_mutation_authority_status_not_admissible"]

    reasons: list[str] = []
    if not _question_card_is_explicit(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_question"),
        candidate_path,
    ):
        reasons.append(
            "stack_command_implementation_actual_owner_side_mutation_authority_question_not_explicit"
        )
    if _normalized_string_list(
        bundle.get("stack_command_implementation_actual_owner_side_mutation_authority_reasons")
    ):
        reasons.append("stack_command_implementation_actual_owner_side_mutation_authority_reasons_present")
...
def evaluate_supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_surface(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    result["stack_command_implementation_actual_owner_side_mutation_surface_status"] = (
        STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE_STATUS_ADMISSIBLE
        if not reasons
        else NO_STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_SURFACE
    )
    result["stack_command_implementation_actual_owner_side_mutation_surface_question"] = (
        _question_card(candidate_path) if not reasons else None
    )
    result["stack_command_implementation_actual_owner_side_mutation_surface_reasons"] = reasons
    return result
