...
CONCRETE_COMMAND_FILE_SELECTION_STATUS_ADMISSIBLE = (
    "concrete_command_file_selection_admissible"
)
NO_CONCRETE_COMMAND_FILE_SELECTION = "no_concrete_command_file_selection"
QUESTION_PROMPT = (
    "Which later admitted surface, if any, can choose the concrete command file for "
    "`stack supervised-execution-home` without choosing one runtime home, one _stack "
    "command implementation surface, one worker authority class, one owner-repo edit "
    "authority, one actual owner-side mutation authority, one Playbook doctrine export "
    "path, or one protected-surface exception?"
)
...
def _selection_reasons(
    ...
    if (
        ...
        reasons.append("concrete_stack_command_home_selection_status_not_admissible")
...
def evaluate_supervised_execution_home_concrete_command_file(
    bundle: Mapping[str, Any],
) -> dict[str, Any]:
