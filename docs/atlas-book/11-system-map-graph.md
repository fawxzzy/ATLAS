...
- `No immediate ATLAS-root packet is open`
  ...
  - prior active-family carry-forward: `Sandbox Simulation Readiness post-local-only first validator broader-runtime-assertions admission boundary hold or top-level lane reselection` remains restart-relevant history, but it is no longer an immediate ATLAS-root packet because the root now explicitly freezes `No immediate ATLAS-root packet is open`
