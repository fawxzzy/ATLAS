...
- latest ATLAS-root immediate lane packet:
  ...
  - the current selector helper now emits the explicit operator action `continue_current_lane`, cites the decisive receipt for that current packet, and also cites the decisive receipt for the downstream fallback `AI Long-Run Batch Orchestration queue-or-registry active follow-on`
