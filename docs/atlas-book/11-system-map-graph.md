...
| --- | --- | --- | --- | --- | --- |
| Fitness app lane | Fitness | `repos/fawxzzy-fitness` plus protected QA read-model receipts | adopted protected-QA topology is repaired and current: `playbook`, `trove`, `foundation`, and `lifeline` are release-ready, `fitness` is now blocked on current SHA `12d726b57edc1c8cb81128eac9e47daabec97cd1` with the published inventory also showing the repo dirty on `main`, `stream` is visible as `not_applicable` because it is not release-eligible, and the BrowserStack provider control plane remains valid for `desktop.chromium.real`, `android.chrome.real`, and `iphone.webkit.real` | the current protected-refresh republish is no longer blocked by stale command routing; the remaining live blocker is one owner-side `iphone.webkit.emulated` visual diff on the dirty current Fitness worktree plus the still-open `desktop.chromium.real`, `android.chrome.real`, and `iphone.webkit.real` real-device/manual requirement, while BrowserStack credentials are absent locally even though the provider control plane remains valid | Owner-side Fitness visual/baseline decision first, then desktop, Android, and iPhone real-device proof or manual attestation for `fitness`, or one protected BrowserStack run once real credentials are present; reopen `stream` only if stack governance later promotes it into release scope |
