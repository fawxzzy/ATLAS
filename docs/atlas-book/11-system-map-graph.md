...
| --- | --- | --- | --- | --- | --- |
| Fitness app lane | Fitness | `repos/fawxzzy-fitness` plus protected QA read-model receipts | adopted protected-QA topology is repaired and current: `playbook`, `trove`, `foundation`, and `lifeline` are release-ready, `fitness` is now blocked on current SHA `12d726b57edc1c8cb81128eac9e47daabec97cd1` with the published inventory also showing the repo dirty on `main`, `stream` is visible as `not_applicable` because it is not release-eligible, and the BrowserStack provider control plane remains valid for `desktop.chromium.real`, `android.chrome.real`, and `iphone.webkit.real` | the current protected-refresh republish is blocked by owner-side Fitness execution failure rather than stale provider support: the current run surfaces `next` command-resolution failure plus `DevMobileRegressionRoute.tsx` `TS2448` and `TS2454`; no live release blocker remains for `stream` while it stays out of release scope | Owner-side Fitness execution repair first, then Android and iPhone real-device proof or manual attestation for `fitness`, or one protected BrowserStack run with real credentials; reopen `stream` only if stack governance later promotes it into release scope |
