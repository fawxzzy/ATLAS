...
- ATLAS root now also owns one bounded local-only Sandbox simulation substrate under `data/atlas/sandbox/**` and `runtime/atlas/sandbox/**`, but that substrate remains contract-only for now and still admits no validator, runner, `_stack`, owner-repo, deploy, secret, or live-data widening
