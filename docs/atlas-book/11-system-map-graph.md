...
| --- | --- | --- | --- | --- | --- |
| Fitness app lane | Fitness | `repos/fawxzzy-fitness` plus protected QA read-model receipts | adopted protected-QA topology is repaired and current: `playbook`, `trove`, `foundation`, and `lifeline` are release-ready, `fitness` holds clean emulated proof at `manual_review` on current SHA `9403472d200e7d620fc1ba8e00d6d9509f00510f`, `stream` is visible as `not_applicable` because it is not release-eligible, and the BrowserStack provider control plane is now valid for `desktop.chromium.real`, `android.chrome.real`, and `iphone.webkit.real` | `fitness` still needs current-run physical/manual proof for `android.chrome.real` and `iphone.webkit.real`; the remaining provider-side blocker is real credential availability rather than stale provider support; no live release blocker remains for `stream` while it stays out of release scope | Android and iPhone real-device proof or manual attestation for `fitness`, or one protected BrowserStack run with real credentials; reopen `stream` only if stack governance later promotes it into release scope |
