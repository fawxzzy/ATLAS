...
- latest ATLAS-root immediate lane packet:
  ...
  - that selector is now durable and freezes `AI Long-Run Batch Orchestration single supervised pilot selected-pilot implementation-routing contract freeze pass 487` as the exact next package without reopening owner-repo mutation authority or `_stack` execution-home doctrine
