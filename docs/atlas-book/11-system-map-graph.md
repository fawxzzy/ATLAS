...
- latest ATLAS-root immediate lane packet:
  ...
  - the current selector helper now renders that active packet separately from the first downstream admissible follow-on, which is currently `AI Long-Run Batch Orchestration queue-or-registry active follow-on`
