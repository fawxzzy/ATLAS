...
- ATLAS root now also owns one bounded local-only Sandbox simulation substrate under `data/atlas/sandbox/**` and `runtime/atlas/sandbox/**`; that substrate now includes one admitted example scenario manifest plus one paired empty fixture-pack stub, but it still admits no leaf fixture payload, validator, runner, `_stack`, owner-repo, deploy, secret, or live-data widening
