...
| --- | --- | --- | --- | --- | --- |
| Fitness app lane | Fitness | `repos/fawxzzy-fitness` plus protected QA read-model receipts | adopted protected-QA topology is repaired and current: `playbook`, `trove`, `foundation`, and `lifeline` are release-ready, `fitness` holds clean emulated proof plus a valid desktop real-browser manual attestation at `manual_review`, and `stream` remains the only trusted-origin-blocked repo | `fitness` still needs mobile real-device proof for `android.chrome.real` and `iphone.webkit.real`; `stream` still needs either trusted release-origin evidence or explicit non-release governance | Android and iPhone real-device proof or manual attestation for `fitness`; `stream` protected-origin conversion or explicit release-governance classification |
