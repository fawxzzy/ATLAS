...
- `Cortex Readiness` now has one bounded read-model freshness proof:
  ...
  - the canonical inventory spine now also absorbs that closeout cluster directly: the immediate lane is `Inventory & Truth Map`, the supporting lane is `Truth Map & ATLAS Book`, and the current held-family set is recoverable from the root inventory surfaces rather than only from adjacent receipts plus chat-held coordination
