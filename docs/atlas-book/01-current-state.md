...
- `Cortex Readiness` now has one bounded read-model freshness proof:
  ...
  - the immediate untracked control-plane carry is now also split explicitly: the restart-referenced untracked `docs/ops/*` tranche already cited by the active restart spine is preserved, the next exact preservation target is the colder untracked `LOCAL-DATA-GATEWAY-*` checkpoint family, the two-file non-LDG colder `docs/ops/*` tail remains later adjacent hold, and retained `archive/*` evidence also remains later adjacent hold
