...
- `Cortex Readiness` now has one bounded read-model freshness proof:
  ...
  - the immediate untracked control-plane carry is now also split explicitly: the next exact preservation target is the restart-referenced untracked `docs/ops/*` tranche already cited by the active restart spine, while colder untracked `docs/ops/*` receipts remain later adjacent hold with retained `archive/*`
