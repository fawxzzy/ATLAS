...
- `Cortex Readiness` now has one bounded ATLAS catch-up and root-projection proof plus one bounded post-catch-up live-lane ratchet proof:
  ...
  - PR `#55` is now the durable review surface for that bounded projection-refresh tranche: `open`, `ready for review`, `mergeable`, `unmerged`, with `0` review submissions and `0` review threads
