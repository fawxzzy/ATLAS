...
- `Cortex Readiness` now has one bounded read-model freshness proof:
  ...
  - the immediate untracked control-plane carry is now also split explicitly: the restart-referenced untracked `docs/ops/*` tranche is preserved, the colder untracked `LOCAL-DATA-GATEWAY-*` checkpoint family is preserved, the next exact preservation target is the two-file non-LDG colder `docs/ops/*` tail, and retained `archive/*` evidence remains later adjacent hold
