...
- `Cortex Readiness` now has one bounded read-model freshness proof:
  ...
  - `Dependency Untangling` is now the active ATLAS-side structural lane from that refreshed substrate: the verification bridge seam no longer depends on Fitness repo/runtime repair after the post-install bridge-timeout boundary receipt, the remaining blocker is explicitly external/session-scoped Codex-to-Chrome bridge state, and `Atlas-owned Repo Naming Canonicalization` stays supporting only if a direct naming or path dependency later appears
