...
- `AI Repetition-to-Automation Pipeline` now has one automation-candidate threshold packet:
  ...
  - the preserved `_stack` publication tranche is now also on a normal remote branch pair: `main` and `codex/preserve-stack-packaging-tranche-2026-06-05` both exist remotely at the same `eb1f7c4` commit, `_stack_remote_not_configured` is cleared, no meaningful PR surface remains because the branches are identical, and the only remaining publication-posture step is switching the GitHub default branch from the codex branch to `main`
