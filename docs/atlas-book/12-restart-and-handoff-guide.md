...
- the exact next move after preserving that tranche is blocker-facing classification between the later memory-path canonicalization carry `docs/memory/initiatives/initiative-mazer-d2-learning-scorer.json` and the residual QA workflow carry `.github/workflows/atlas-qa-llel.yml`; retained `archive/*` evidence remains outside that decision
