...
- Durable Context Externalization now sits at `77%` because the broadened retrieval substrate is no longer only broader on paper; the lane now also externalizes the current active execution-state spine itself so restart no longer depends on chat-held lane ordering and held-lane memory alone, but it still stays below higher territory because continuity coverage is still partial, refresh discipline is still operator-driven, and some restart paths still depend on manual interpretation across receipt chains
