...
- next valid package:
  - explicit Fitness lane reopen, or approved Fitness Supabase Mutation Pass 1 only
...
- currently open for continued doctrine promotion
...
- currently open for planned command design and later implementation
...
- currently open because the durable substrate is already strong, the first real continuity manifests are now durably seeded for the first-adoption major lane set, and restart routing can point to real manifest-backed retrieval maps, but retrieval-first use is still partly manual and not yet universal across all major lanes
- next valid package:
...
- currently open at proof-backed full local lifecycle stage plus thin wrapper package 1 through real wrapper-layer proof for `validate-only` and `emit-dry-run`
- next valid package:
...
- Lifeline-facing health signal design
