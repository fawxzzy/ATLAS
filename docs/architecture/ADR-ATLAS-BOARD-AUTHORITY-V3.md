...
All schemas declare JSON Schema Draft 2020-12 and have schema-valid plus semantic-invalid focused fixtures. Semantic validation is part of the registered artifact validator. Primary schema-invalid artifacts return deterministic validation errors before semantic dereference or any runtime/projection context read; the validator enforces both `anyOf` and `oneOf` branches used by the registered family. Projection acknowledgements are context-dependent admission artifacts: the registered CLI requires the referenced `ProjectionDelivery`, validates that delivery, and rejects the acknowledgement unless every correlation field and payload digest match. `BoardCommitReceipt` is also context-dependent: admission requires validated referenced `CardEvent` and `ProjectionDelivery` artifacts and exact correlation of card, project, board, event, sequence, version, expected version, event idempotency key, receipt/delivery/lease identities, materialized-record digest, execution-receipt identity and digest, canonical whole-delivery digest, and event-to-delivery-to-commit chronology. Canonical artifact digests hash compact UTF-8 JSON after recursively sorting object keys while preserving array order, with no extra whitespace or terminal line feed; the whole-delivery digest therefore binds its delivery idempotency key and payload digest without equating either to the event's transaction idempotency key.
...
An existing idempotency key returns the original accepted result and creates no second event, version, outbox row, or receipt. An expected-version mismatch rejects before append and creates no materialization, delivery, or accepted receipt. Any failure before commit rolls back all transaction-owned effects. The accepted BoardCommitReceipt repeats the admitted event sequence/version relation and rejects a sequence that precedes card version.

Event sequence is monotonic and immutable, never precedes the resulting card version, and card version equals `expected_version + 1`. Initial events use expected version zero; every non-initial event requires an existing version-one-or-greater preimage. Transition endpoints are restricted to the CardRecord lifecycle domain; only an initial transition may start from null, and no transition may target null or an unknown lifecycle. When both lifecycle `set` and `transition` operations are present, the materialized lifecycle must equal the transition target; conflicting operations reject before append. Initial event admission deterministically projects the complete CardRecord from the envelope and full set, gives the succeeded execution receipt its known status only when both receipt identity and digest match, gives other statusless receipt references `UNKNOWN`, then applies the CardRecord semantic validator before append. Any added reference reusing the execution-receipt identity with another digest rejects. Non-initial admission rejects lifecycle, archive-state, and standing-anchor combinations only when the event supplies enough affected fields to prove a contradiction; missing preimage fields are never invented. A transition crossing the archived boundary supplies the corresponding archive state, and entry into archived also supplies `standing_anchor: false`, because the transition endpoints prove those post-state fields are required. Added blocker, resource, and receipt identities are unique within an event, and one event cannot both add and remove the same blocker or resource. Replay orders events by event sequence, applies the frozen deterministic operations, verifies each resulting record digest, and fails closed on gaps, duplicates, version drift, or digest drift. JSONL export uses canonical UTF-8/LF serialization and is the canonical portable audit/export form; it is not transaction authority.
...
Visible projection states are exactly `queued`, `applied`, `stale`, `failed`, and `UNKNOWN`. Unavailable evidence is always `UNKNOWN`; it is never converted to zero requests, healthy state, applied state, or empty-board truth. Unavailable delivery and acknowledgement artifacts retain non-empty availability-error evidence while request count, observation time, and response digest stay null. When the adapter is available but the result is `UNKNOWN`, touched-card readback must also be `UNKNOWN` and retain positive request count, observation time, and response digest evidence.

Every delivery and acknowledgement binds card, event, sequence, version, idempotency key, attempt count, and payload digest. Queued delivery is retryable, has attempt count zero, and retains no error. Applied, stale, failed, and available-`UNKNOWN` deliveries record a positive attempt count; unavailable-`UNKNOWN` delivery stays at zero because unavailable evidence cannot invent an attempt. Failed delivery or acknowledgement retains non-empty error evidence; applied state retains none. Every acknowledgement records a positive attempt count. Critical-path readback fetches the exact touched card only and records request count plus response digest. Full-board scans are scheduled integrity work outside engineering closure. A partial projection failure retries only the failed delivery; an applied delivery or acknowledgement is non-retryable and has no next attempt. Retry never replays the accepted local transaction or unrelated cards.

Delivery readback never predates enqueue. An acknowledgement never predates its referenced delivery enqueue, and any acknowledgement readback falls between enqueue and acknowledgement. Generated Atlas Control cards are unique by `(project_id, board_id, card_id)`; card count and projection summaries are computed from that deduplicated identity set.
...
The complete baseline object—base commit, selection rule, file count, path digest, tree digest, and ordered protected path groups—is frozen in `docs/registry/ATLAS-BOARD-AUTHORITY-MIGRATION.v1.json`; registered migration/import/idempotency/cutover identities are immutable too. The live v2 authority snapshot remains `UNKNOWN` in this packet. ATLAS-BOARD-001 must capture an exact content-addressed source snapshot, verify it, and import it once under the registered import id and idempotency key. Baseline import emits deterministic v3 events and must prove byte-identical replay. Import completion and import verification are separate recorded milestones. Migration chronology is monotonic: contract creation precedes snapshot capture, snapshot capture precedes import, import precedes verification, and verification precedes the first accepted v3 receipt. The first-acceptance record stores the exact accepted `BoardCommitReceipt` identity, canonical digest, and commit timestamp; admission revalidates that receipt against its referenced event and delivery before accepting the authority boundary.
