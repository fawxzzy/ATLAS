# AI Long-Run Batch Orchestration Queue-Or-Registry Broader Attention-Queue Unknown-Extension-Surface Contract Freeze Pass 391 - 2026-06-17
...
- Source surfaces:
  ...
  - `docs/ops/AI-LONG-RUN-BATCH-ORCHESTRATION-QUEUE-OR-REGISTRY-POST-BROADER-ATTENTION-QUEUE-UNKNOWN-TOOL-SURFACE-NEXT-SLICE-SELECTION-PASS-390-2026-06-17.md`
...
- Control-plane checkpoint: `main@5aaf6fed`

## Objective

Freeze the exact bounded contract for the already-live `unknown_extension_surface` item family inside the broader root-owned `attention_queue` surface while preserving the queue behavior already landed on canonical `main`.
...
- reinterpret governed-surface extension-id contradiction as repair authority, registry mutation authority, retry authority, resume authority, merge authority, or supervisor proof
- reopen the already-frozen sibling `unknown_tool_surface` family as part of the same packet
...
- the reconciled `unknown_tool_surface` slice proving one compact root-local governed-surface contradiction follow-on landed cleanly
- pass 390 selection of `unknown_extension_surface` as the next bounded live governed-surface contradiction follow-on
...
4. `extension_id` is a non-empty string after trim
5. the trimmed `extension_id` is not present in `registry_state.extension_ids`
6. the family is describing governed-surface extension-id contradiction visibility only, not performing registry repair, registry mutation, retry dispatch, resume execution, merge execution, or owner-repo mutation
...
- per-scope `extension_id`
- per-scope `tool_id` only as carried context
- the admitted `registry_state.ok` payload
- `registry_state.extension_ids`
...
- tool-id contradiction alone without the unknown-extension branch qualifying
...
- `severity = "high"`
- `summary = "{scope} references unknown extension_id '{extension_id}'."`
...
For this family, `details.tool_id` may be either the trimmed tool id string or `null` when no tool id is present.
...
This means `unknown_extension_surface` coexists under the same severity-first then `kind` then `source_ref` then `summary` ordering already frozen for the broader queue.

## Exact Qualification Split

The governed-surface extension contradiction family coexists with neighboring registry and contradiction families exactly as follows:

- only `unknown_extension_surface` emits extension-id contradiction visibility
- `unknown_tool_surface` remains the separate sibling tool-id contradiction family
...
- if `extension_id` is missing, empty after trim, or already present in `registry_state.extension_ids`, this family stays silent

This contract freeze names that split only.
It does not reopen the already-frozen sibling tool contradiction family itself.

## Exact Registry-Interaction Guard

This family requires healthy registry state and one unknown trimmed `extension_id` value to qualify.

The exact coexistence guard remains:

- `unknown_extension_surface` does not emit when `registry_state.ok` is falsey
- `registry_error` may emit separately when registry health is unavailable
- `unknown_extension_surface` may coexist with `unknown_tool_surface` when the same governed surface carries both an unknown extension id and an unknown tool id
- `unknown_extension_surface` may coexist with `registry_drift`, `session_needs_resume`, `resume_failed`, or `session_failed` when those admitted active-session families also qualify
...
- `unknown_extension_surface` does not alter `PROVENANCE_ALERT_QUEUE_SIGNAL_CAP`
...
- its producing truth comes from root-owned active-session governed-surface summaries plus root-loaded registry extension ids already surfaced inside ATLAS-root control-plane code
...
Governed-surface extension-id contradiction visibility is not:
...
- tool contradiction authority
- owner-repo mutation authority

This family only surfaces one bounded governed-surface unknown-extension sentinel inside the broader operator-review queue.
...
- `AI Long-Run Batch Orchestration queue-or-registry broader attention_queue unknown_extension_surface owner-surface admission pass 392`
...
`Unknown Extension Surface Contract Drift`

If the lane leaves the already-live `unknown_extension_surface` seam informal, later workers can reopen qualification, payload shape, severity, or sibling-contradiction coexistence questions that should have been frozen before ownership or implementation routing advances.
