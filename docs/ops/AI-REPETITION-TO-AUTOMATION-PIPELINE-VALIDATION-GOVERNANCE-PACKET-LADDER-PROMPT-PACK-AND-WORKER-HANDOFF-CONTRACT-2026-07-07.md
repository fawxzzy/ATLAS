...
Use the already-landed generic packet-ladder helper to produce deterministic packet-ladder proof for the accepted `validation-governance` review card.

This worker does not implement validation logic, governance verdicts, release readiness, or platform checks. The objective is bounded to proving that the existing root-only packet ladder can safely package the broad validation/governance candidate family without widening authority.
...
- live helper output returns `candidate_id=validation-governance`
...
- the worker would need to infer validation verdicts, release readiness, or governance truth beyond the helper output
