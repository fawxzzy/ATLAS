# AI Long-Run Batch Orchestration Queue-Or-Registry Broader Attention-Queue Session-Failed Prompt-Pack And Handoff Contract Pass 374 - 2026-06-16
...
- Control-plane checkpoint: `main@61fbd495`

## Objective

Freeze one compact authoritative prompt-pack and handoff contract for the already-admitted first implementation slice of the root-local `session_failed` family inside the broader `attention_queue` render-status surface.
...
- pass 370 already froze the exact `session_failed` contract around admitted active-session qualification, bounded queue payload only, fixed severity, deterministic ordering, and separation from retry execution, resume execution, merge execution, registry repair, or owner-repo authority
- pass 371 already admitted `ATLAS root control-plane surfaces` as the owner-facing home for this seam
- pass 372 already proved separate support still honestly holds at `none yet`
- pass 373 already froze the exact first implementation slice around admitted active-session qualification, queue emission, inherited deterministic queue merge, unchanged top-level handoffs, and the exact proof matrix
...
- pass 370 exact `session_failed` contract
- pass 371 root control-plane owner admission
- pass 372 supporting-lane hold at `none yet`
- pass 373 exact first implementation slice and exact proof matrix
...
- implement or narrowly reconcile the already-admitted root-local `session_failed` first slice inside `ops/cortex/render_status.py` and `tests/test_cortex_render_status_provenance.py` so it preserves admitted active-session qualification through `session_state` plus `final_status` only, preserves one fixed `session_failed` queue item with no widened detail payload beyond `session_id` and `task_id`, preserves fixed `high` severity, preserves inherited deterministic `attention_item_sort_key(...)` queue ordering against the already-admitted broader queue families, preserves queue emission when the admitted active-session state qualifies even if registry health is unavailable while preserving separate `registry_error` coexistence only, preserves separate `registry_drift` coexistence when the active-session registry digest differs from the current registry digest, preserves the unchanged top-level `attention_queue` and `active_session` handoffs through `render_status_payload(...)`, and proves behavior against the frozen pass-373 matrix
...
2. the inherited pass 290 plus passes 370 through 373 as frozen inputs
...
- `AI Long-Run Batch Orchestration queue-or-registry broader attention_queue session_failed implementation-readiness closeout and worker-routing pass 375`
...
Freeze the worker handoff contract before authorizing bounded implementation work for a root-local `session_failed` queue seam that already has its contract, owner, support posture, and first slice admitted.
