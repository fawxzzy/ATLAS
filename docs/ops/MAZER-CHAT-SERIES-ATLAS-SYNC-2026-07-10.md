...
- Recovery-target ledger: D considers `29.856` frontier candidates per recovery on average, compared with E's `23.175`; selected-score margins are near-tied (`0.328` D, `0.275` C). The next D trial is a committed-frontier/candidate-pool policy measured against raw route ratio, not another global path penalty.
