# AI Long-Run Batch Orchestration Queue-Or-Registry Broader Attention-Queue Unknown-Tool-Surface Prompt-Pack And Handoff Contract Pass 388 - 2026-06-17
...
- Control-plane checkpoint: `main@2c2cf2d5`

## Objective

Freeze one compact authoritative prompt-pack and handoff contract for the already-admitted first implementation slice of the root-local `unknown_tool_surface` family inside the broader `attention_queue` render-status surface.
...
- reopen `_stack`, Playbook, registry-repair doctrine, registry-mutation doctrine, sibling-contradiction doctrine, or owner-repo support
- widen into registry repair, registry mutation, retry execution, resume execution, merge execution, sibling `unknown_extension_surface` ownership, or broader active-session semantics
- infer unknown-tool truth from hidden transcript state instead of cited durable surfaces
- touch protected backlog under `archive/`, `.vercel`, `.env*`, `secrets/`, screenshots, captures, or broad untracked root docs and media

## Root Health Baseline

- pass 384 already froze the exact `unknown_tool_surface` contract around healthy registry plus trimmed unknown-tool qualification, bounded queue payload only, fixed severity, deterministic ordering, and separation from registry repair, registry mutation, retry execution, resume execution, merge execution, sibling contradiction doctrine, or owner-repo authority
- pass 385 already admitted `ATLAS root control-plane surfaces` as the owner-facing home for this seam
- pass 386 already proved separate support still honestly holds at `none yet`
- pass 387 already froze the exact first implementation slice around governed-surface plus registry qualification, queue emission, inherited deterministic queue merge, unchanged top-level handoffs, and the exact proof matrix
...
- pass 384 exact `unknown_tool_surface` contract
- pass 385 root control-plane owner admission
- pass 386 supporting-lane hold at `none yet`
- pass 387 exact first implementation slice and exact proof matrix
...
- implement or narrowly reconcile the already-admitted root-local `unknown_tool_surface` first slice inside `ops/cortex/render_status.py` and `tests/test_cortex_render_status_provenance.py` so it preserves qualification through `active_session.governed_surfaces` plus per-scope `tool_id` plus `registry_state.ok` plus `registry_state.tool_ids` only, preserves one fixed `unknown_tool_surface` queue item with no widened detail payload beyond `scope`, `tool_id`, and `extension_id`, preserves fixed `high` severity, preserves inherited deterministic `attention_item_sort_key(...)` queue ordering against the already-admitted broader queue families, preserves omission when the tool id is known, missing, empty, or the admitted scope payload is not a dict, preserves omission when registry health is unavailable while preserving separate `registry_error` coexistence only and fail-closed contradiction omission, preserves separate coexistence with sibling `unknown_extension_surface` plus `session_needs_resume`, `resume_failed`, and `session_failed` when those admitted families also qualify, preserves the unchanged top-level `attention_queue` and `active_session` handoffs through `render_status_payload(...)`, and proves behavior against the frozen pass-387 matrix
...
- broader active-session or doctrine semantics
...
- queue payload:
  ...
  - `highest_severity`
  - `items`
- `unknown_tool_surface` item payload:
  - `kind`
  - `severity`
  - `summary`
  - `source_ref`
...
The worker may not widen them into repair authority, mutation authority, retry authority, resume authority, merge authority, sibling-contradiction projection, queue-budget metadata, or broader operator semantics.
...
6. coexistence with admitted active-session state families
   ...
   - do not widen the governed-surface contradiction payload into resume, retry, repair, or failure semantics

7. mixed unknown-tool plus other broader queue families
...
8. top-level render-status handoff
   ...
   - do not widen those handoffs into repair authority, mutation authority, retry authority, resume authority, merge authority, or doctrine semantics
...
`No-mutation guard: this packet may admit future implementation or bounded reconciliation of one active-session governed-surface plus registry-state qualification layer using active_session.governed_surfaces plus per-scope tool_id plus registry_state.ok plus registry_state.tool_ids only, one unknown_tool_surface emission branch inside the governed-surface contradiction helper used by attention_queue(...), one inherited deterministic attention_item_sort_key(...) queue merge layer, and one unchanged top-level render_status_payload(...) handoff for attention_queue plus active_session, but it may not mutate queue, registry, runtime, session, execution-receipt, merge, manifest, or owner-repo state, change queue-budget or overflow behavior, widen into registry repair, registry mutation, retry execution, resume execution, merge execution, broader contradiction-family mutation, or imply supervisor/operator proof.`
...
- broader repair, mutation, retry-execution, resume-execution, merge-execution, contradiction, supervision, dispatch, or execution-home surfaces
...
- new payload fields, new status values, new failure states, reordered item families, or non-deterministic sort behavior
...
2. the inherited pass 290 plus passes 384 through 387 as frozen inputs
...
- `AI Long-Run Batch Orchestration queue-or-registry broader attention_queue unknown_tool_surface implementation-readiness closeout and worker-routing pass 389`
...
Freeze the worker handoff contract before authorizing bounded implementation work for a root-local `unknown_tool_surface` queue seam that already has its contract, owner, support posture, and first slice admitted.
...
`Unknown Tool Surface Handoff Drift`

If the worker handoff contract stays implicit, the already-admitted unknown-tool seam expands through prompt wording into repair, mutation, retry execution, resume execution, merge execution, sibling contradiction, overflow, hidden-state, protected-backlog, or broader runtime semantics that the durable chain has not admitted.
