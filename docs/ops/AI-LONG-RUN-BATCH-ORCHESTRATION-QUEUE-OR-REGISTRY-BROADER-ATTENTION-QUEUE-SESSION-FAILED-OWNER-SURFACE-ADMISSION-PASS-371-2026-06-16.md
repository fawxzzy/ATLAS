# AI Long-Run Batch Orchestration Queue-Or-Registry Broader Attention-Queue Session-Failed Owner-Surface Admission Pass 371 - 2026-06-16
...
- Control-plane checkpoint: `main@8f2876bf`

## Objective

Freeze the exact owner-facing home for the bounded `session_failed` queue seam after the contract itself is explicit.
...
- broader registry-drift and contradiction-family ownership
...
- root may retry failed session flows
...
- `AI Long-Run Batch Orchestration queue-or-registry broader attention_queue session_failed supporting-lane admission pass 372`
...
If a terminal session-failure queue family is still produced and consumed entirely inside ATLAS-root status helpers and restart surfaces, keep ownership root-local until a real shared helper or owner-repo dependency actually appears.
