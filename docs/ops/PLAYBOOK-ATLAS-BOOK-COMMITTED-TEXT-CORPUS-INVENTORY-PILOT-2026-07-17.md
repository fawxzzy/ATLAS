...
- Secret, credential, and token stems, including dot-prefixed filenames, across every supported configuration extension, and `secret`/`secrets` directory segments with an optional leading dot, plus private/transcript, runtime, temporary, dependency/vendor, build/generated, symlink, gitlink, binary, and unsupported-media surfaces, fail closed before blob reads. Ordinary hidden directories and credential/token product directories are not broadened into that segment policy. Excluded bodies are not stored and their content SHA-256 remains `UNKNOWN`.
