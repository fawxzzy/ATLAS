...
4. Inspect `runtime/atlas/workflow-recovery/plan.json`, optional `post-apply-plan.json`, `creation-journal.json`, `live-registry.json`, and `RECEIPT.json`. Live apply must use this canonical runtime directory; an alternate output directory or `--no-write-runtime` fails before discovery or mutation. `plan.json` remains the immutable accepted pre-mutation plan. Immediately before every `CREATE`, the adapter performs complete discovery and rebuilds that logical role's accepted decision; a new claimant, lease collision, or any other decision drift fails closed with no creation. Immediately after each successful `CREATE`, `creation-journal.json` is durably replaced with a content-addressed snapshot retaining the exact runtime ID before any later title, policy, pin, or bootstrap action can run. The temporary file is fsynced before replacement. POSIX systems then fsync the modified parent directory; Windows uses `MoveFileExW` with replace-existing and write-through flags. Unsupported platforms and failed file, replacement, or directory synchronization stop later actions. A fresh process validates and loads the binding before planning; if complete discovery cannot return the exact ID, recovery blocks without another create. After apply, `post-apply-plan.json` records the separate content-addressed readback plan used for terminal health and registry output. `RECEIPT.json` binds the accepted plan, post-apply plan, and journal event/digest. Runtime output is intentionally ignored by Git. Plan digests always exclude `generated_at`; when an observation is present they also exclude volatile receipt identity, source host, and timestamps. The validated activity effects, recovery decisions, cwd locator, and any resolved creation/bootstrap cwd remain digest-bound.
...
The test lane covers healthy, missing, stale-ID, duplicate, active-writer, partial-create, fresh-process retry, and unknown-state cases. Keep runtime output enabled when proving crash recovery; `--no-write-runtime` is fixture-only and intentionally cannot prove journal durability. Live apply rejects that flag. A successful create preserves the accepted plan unchanged while atomically retaining the runtime ID, then carrying the read-back binding into `post-apply-plan.json` and `live-registry.json`; a missing post-apply runtime fails closed. The terminal report uses the post-apply health rather than the pre-mutation status. A partial create is retained even if the immediately following action fails. A fresh process either discovers and repairs the exact journaled runtime or blocks on that exact ID, and never creates a second runtime.
