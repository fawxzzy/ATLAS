...
Normal routes are direct and owner-returning: owner -> Release -> owner and owner -> Manual -> owner. DiscordOS is archived and retired provenance only: historical cards, branches, pull requests, receipts, and compatibility deliveries remain readable, but they can never select, unarchive, wake, dispatch, or publish through DiscordOS. Future project-board product work routes to `owner.fawxzzyweb`, and master-data work routes to `platform.supabase-migration`, only after each exact owner or data contract is proven. `atlas.inbox` receives no routine aggregation copy. An already-existing compatibility delivery remains readable, but failed or idle Inbox state never blocks owner progress.
...
Every material wake consumes all canonically authorized `READY` standing packets in dependency order and dispatches the largest conflict-free wave. A standardized packet is recognized through one normalized, duplicate-free policy set derived from either legacy `policy_id` or current `policy_ids`; the Workflow Standardization policy cannot be bypassed by using the plural form. It must carry two independently validated closed identities: `execution_target` names the current logical role, runtime epoch, and non-empty host that receives the work, while `owner_return` names the current source-owner callback. They may be different for owner -> Release -> owner and owner -> Manual -> owner routes. The scheduler binds both identities, the target role, source runtime, event, and payload into `transport_digest`; replaying the same event while changing a target epoch, callback epoch, or host is rejected. A standing execution target in `idle` or `notLoaded` state is resumed through that current binding. An active target is never steered: its packet remains queued until the target reaches a safe boundary, and the watchdog emits `BLOCKED_QUEUE` instead of interrupting it. `component.discordos` is never an execution target or owner callback.
...
Every terminal packet resolves to exactly one closed successor: `NEXT_AUTONOMOUS_PACKET`, `MANUAL_REQUIRED`, `EXTERNAL_WAIT`, `TERMINAL_DOMAIN`, or `ERROR_RECOVERY`. Standardized terminal packets identified by singular or plural policy metadata must carry that explicit value; omission or a value outside the set is rejected. Legacy receipts may use only exact normalized closed states, never substring inference, so text such as `MANUALLY_VERIFIED_PASS`, `SUCCESS_NO_ERRORS`, or `UNKNOWN_ITEMS_ZERO` cannot accidentally select a blocking successor. A free-form tracking line cannot select a successor. Terminal settlement is driven by that validated successor plus explicit blocking authority, never lifecycle-state substrings: a non-blocking `TERMINAL_DOMAIN` or `NEXT_AUTONOMOUS_PACKET` receipt enters exact completion, while every other successor enters its state-specific wait or recovery handling; `blocking=true` retains the correlated mutating packet, intent, and lease. A non-completing read-only receipt retains its exact standing packet and delivered intent without consuming a writer lease, records one exact wake condition beside the content-addressed receipt identity, and projects only into `WAITING_ON_ZAC`, `WAITING_EXTERNAL`, or `BLOCKED_ERROR` rather than `DONE_RECENTLY`. A cross-role packet cannot disappear or release its lease until its direct owner-return phase is `DELIVERED` with the exact owner turn and immutable first-delivery proof. Each valid completion receipt then releases only the exact correlated lease and immediately triggers selection of the next admitted conflict-free wave. `MANUAL_REQUIRED` names the stable question/answer event; `EXTERNAL_WAIT` names the external wake; `ERROR_RECOVERY` emits a content-addressed recovery packet. `BLOCKED`, `REVIEW_LATENCY`, `UNKNOWN`, or an active lease suppresses only that conflict group.

Every incoming workflow receipt or operator answer also passes a content-addressed continuation audit before it can be consumed. The audit requires explicit source, target, owner-return, correlation, lifecycle, and wake identities bound to current task epochs. It resolves exactly one outcome: `CONTINUE_AUTHORIZED`, `ROUTE_REVIEW`, `ROUTE_MANUAL`, `WAIT_EXTERNAL`, `ERROR_RECOVERY`, or `VERIFIED_TERMINAL_HOLD`. A clean review without separate lifecycle authority routes one ready-only decision to Manual Messages; ready approval continues only the guarded ready transition and merge-decision route; merge approval continues only guarded merge plus post-merge proof; current source authority continues only its bounded source packet. `action_required=false` is advisory and cannot suppress lifecycle or wake semantics. Tool success without exact destination acceptance is `DELIVERY_UNKNOWN`. Only a terminal hold with explicit proof of no admitted successor, unanswered decision, pending owner return, in-flight review, or collision-free ready packet permits the owner lane to stop. Unchanged replay reuses the first audit and successor event; changed digest, idempotency reuse, stale head, wrong owner, or missing wake fails closed.
...
The generated Main portfolio has exactly `DONE_RECENTLY`, `ACTIVE_NOW`, `READY_TO_START`, `WAITING_ON_ZAC`, `WAITING_EXTERNAL`, `BLOCKED_ERROR`, `NEXT_DISPATCHES`, and `HEALTH`. Every lane row identifies role, packet, resource claim, state, last receipt, next executable action, wake condition, owner-return proof/current tracker, and staleness. `HEALTH.scheduler` is `HEALTHY` only when there are zero actionable watchdogs, `BLOCKED` when `ACTIVE_WITHOUT_LEASE`, `MISSING_RUNTIME`, `OWNER_RETURN_UNKNOWN`, or `STALE_ACTIVE_LEASE` is present, and otherwise `DEGRADED`; `blocking_watchdog_count` counts instances while `blocking_watchdog_code_count` counts distinct blocking code classes. Status is derived from durable program state and does not require waking Main.

For an exact source review or publication handoff, reproduce artifact identities from one declared recipe. Encode the sorted relative path denominator as UTF-8 with one LF-delimited path per line and a final LF. Hash the raw `git diff --no-ext-diff --binary <base> -- <ordered paths>` bytes without text normalization. Encode the per-file manifest as a compact canonical JSON object keyed by relative path, where each value has exactly `bytes` and `sha256`; sort keys lexicographically and append one final LF. Encode the source bundle by the same canonical JSON rule and bind its base, current head, branch, path-denominator identity, diff identity, and per-file-manifest identity. Report recipe, byte length, and `sha256:` digest together so an independent reviewer can reproduce each artifact without relying on an unstated local script.
...
10. Initialize the workflow architect and read embedded-service health last. Foundation, Lifeline, Playbook Observer, Cortex, the service bus, ledgers, and heartbeats are components, not automatically required conversations. DiscordOS is retired read-only provenance and is never initialized, selected, or resumed.
...
For components, prove `_stack` operator commands, service-bus dedupe/retry, notification-ledger state, the immutable `component.discordos` retirement guard, Foundation read-model reconstruction, Lifeline wake targets, Playbook Observer read-only behavior, Cortex activation hold, and exactly one heartbeat per declared automation.
