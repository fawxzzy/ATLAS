# AI Long-Run Batch Orchestration Queue-Or-Registry Broader Attention-Queue Registry-Drift Prompt-Pack And Handoff Contract Pass 381 - 2026-06-16
...
- Control-plane checkpoint: `main@c25eda4f`

## Objective

Freeze one compact authoritative prompt-pack and handoff contract for the already-admitted first implementation slice of the root-local `registry_drift` family inside the broader `attention_queue` render-status surface.
...
- reopen `_stack`, Playbook, registry-repair doctrine, contradiction doctrine, or owner-repo support
- widen into registry repair, retry execution, resume execution, merge execution, or broader session-runtime semantics
- infer digest-mismatch truth from hidden transcript state instead of cited durable surfaces
- touch protected backlog under `archive/`, `.vercel`, `.env*`, `secrets/`, screenshots, captures, or broad untracked root docs and media

## Root Health Baseline

- pass 377 already froze the exact `registry_drift` contract around healthy registry plus unequal digest qualification, bounded queue payload only, fixed severity, deterministic ordering, and separation from registry repair, retry execution, resume execution, merge execution, or owner-repo authority
- pass 378 already admitted `ATLAS root control-plane surfaces` as the owner-facing home for this seam
- pass 379 already proved separate support still honestly holds at `none yet`
- pass 380 already froze the exact first implementation slice around active-session plus registry qualification, queue emission, inherited deterministic queue merge, unchanged top-level handoffs, and the exact proof matrix
...
- pass 378 root control-plane owner admission
- pass 379 supporting-lane hold at `none yet`
- pass 380 exact first implementation slice and exact proof matrix
...
- implement or narrowly reconcile the already-admitted root-local `registry_drift` first slice inside `ops/cortex/render_status.py` and `tests/test_cortex_render_status_provenance.py` so it preserves qualification through `registry_state.ok` plus `active_session.registry_digest` plus `registry_state.registry_digest` only, preserves one fixed `registry_drift` queue item with no widened detail payload beyond `session_id`, `session_registry_digest`, and `current_registry_digest`, preserves fixed `high` severity, preserves inherited deterministic `attention_item_sort_key(...)` queue ordering against the already-admitted broader queue families, preserves omission when the digests are equal or when either admitted digest is missing, preserves omission when registry health is unavailable while preserving separate `registry_error` coexistence only and fail-closed contradiction omission, preserves separate coexistence with `session_needs_resume`, `resume_failed`, and `session_failed` when those admitted active-session families also qualify, preserves the unchanged top-level `attention_queue` and `active_session` handoffs through `render_status_payload(...)`, and proves behavior against the frozen pass-380 matrix
...
- broader session-runtime or doctrine semantics
...
The worker may not widen them into repair authority, retry authority, resume authority, merge authority, contradiction-family projection, queue-budget metadata, or broader operator semantics.
...
- broader repair, retry-execution, resume-execution, merge-execution, contradiction, supervisor, dispatch, or execution-home surfaces
...
2. the inherited pass 290 plus passes 377 through 380 as frozen inputs
...
- `AI Long-Run Batch Orchestration queue-or-registry broader attention_queue registry_drift implementation-readiness closeout and worker-routing pass 382`
...
Freeze the worker handoff contract before authorizing bounded implementation work for a root-local `registry_drift` queue seam that already has its contract, owner, support posture, and first slice admitted.
...
`Registry Drift Handoff Drift`

If the worker handoff contract stays implicit, the already-admitted digest-mismatch seam expands through prompt wording into repair, retry execution, resume execution, merge execution, contradiction, overflow, hidden-state, protected-backlog, or broader runtime semantics that the durable chain has not admitted.
