# AI Long-Run Batch Orchestration Queue-Or-Registry Broader Attention-Queue Resume-Failed Contract Freeze Pass 363 - 2026-06-16
...
- Source surfaces:
  ...
  - `docs/ops/AI-LONG-RUN-BATCH-ORCHESTRATION-QUEUE-OR-REGISTRY-POST-BROADER-ATTENTION-QUEUE-SESSION-NEEDS-RESUME-NEXT-SLICE-SELECTION-PASS-362-2026-06-16.md`
...
- Control-plane checkpoint: `main@0cda6513`

## Objective

Freeze the exact bounded contract for the already-live `resume_failed` item family inside the broader root-owned `attention_queue` surface while preserving the queue behavior already landed on canonical `main`.
...
- reinterpret resume-path failure visibility as retry authority, resume execution authority, merge execution authority, registry repair, or owner-repo authority
- reopen broader session-failure, registry-drift, contradiction, or inactive payload doctrine
- infer resume failure truth from hidden transcript state, operator memory, or uncited residue
...
- the reconciled `session_needs_resume` slice proving the bounded active-session follow-up seam, registry coexistence split, and top-level handoff already land cleanly on canonical `main`
- pass 362 selection of `resume_failed` as the next bounded live resume-path follow-on
...
5. either `session_state == "resume_failed"` or `final_status == "resume_failed"`
6. the family is describing explicit resume-path failure visibility only, not performing retry dispatch, resume execution, merge execution, registry repair, or owner-repo mutation

`resume_failure_reason` does not gate emission. It is an admitted detail field only.
...
- `active_session.resume_failure_reason`
...
- hidden transcript-state inference about whether a resume retry should happen
...
This means `resume_failed` coexists under the same severity-first then `kind` then `source_ref` then `summary` ordering already frozen for the broader queue.

## Exact Qualification Split

The resume-failed family coexists with neighboring active-session families exactly as follows:

- only `resume_failed` emits `resume_failed`
- `resume_ready` remains a separate `session_needs_resume` family
- broader `failed` session outcomes remain a separate `session_failed` family
- registry-digest mismatch remains a separate `registry_drift` family
- if neither admitted state field equals `resume_failed`, this family stays silent
...
The `resume_failed` item does not require healthy registry state to qualify once the admitted active-session state already exists.

The exact coexistence guard remains:

- `resume_failed` may emit even when registry health is unavailable
- `resume_failed` may coexist with `registry_drift` when registry health is available and the active-session digest differs from the current registry digest
...
- `resume_failed` does not alter `PROVENANCE_ALERT_QUEUE_SIGNAL_CAP`
- it does not emit or suppress `provenance_alert_overflow`
- it does not widen the top-level queue payload
- it does not reinterpret resume-path failure visibility as queue-budget metadata
...
Resume-path failure visibility is not:
...
- broader failure adjudication authority
- owner-repo mutation authority

This family only surfaces one bounded active-session resume-failure sentinel inside the broader operator-review queue.
...
- `AI Long-Run Batch Orchestration queue-or-registry broader attention_queue resume_failed owner-surface admission pass 364`
...
When a broader `attention_queue` family is already live and root-local, freeze the exact active-session resume-failure qualifier, admitted detail boundary, fixed `high` severity, and separation-from-retry-or-resume-execution rule before reopening ownership, support, or implementation questions.
