...
| raw data lands locally before remote refinement | Operator Secret Path Hygiene, Fitness Supabase Profile/Data Hygiene, AI Repetition-to-Automation | Rule | Prevents privacy spill, token waste, duplicate state, and weak provenance when working with AI, APIs, SaaS tools, or remote databases | redacted Supabase export packets, governed mutation receipts, secret-path cleanup doctrine | `_stack` gateway packet commands, Playbook packet doctrine, future bot export helpers | ATLAS, `_stack`, Playbook, owner repos | gateway receipt plus export manifest and no-secret validation | yes | playbook_now | yes | yes |
