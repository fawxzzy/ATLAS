...
| `foundation` | deferred adjacent control-plane checkout | No active governed dependency surface in the current root checkout. Local path `repos/fawxzzy-foundation` may exist, but it is not admitted in `repo_registry` or `stack_lock.include_repo_ids`; Phase -1 treats it as an explicitly deferred adjacent checkout until manifest, lock, inventory, README, and owner-usage surfaces are updated together. | `light` | `adjacent`, `release_eligible = false`, excluded from governed topology | Keep deferred until an explicit admission package lands; do not let local checkout presence masquerade as managed stack membership |
...
- `foundation` may exist locally under `repos/fawxzzy-foundation`, but local checkout presence is not admission. Phase `-1` keeps it explicitly deferred until all topology surfaces agree.
