...
- `repos/cortex` exists under the root working tree but is not an independent child git root
...
| Repo id | Path | Reason |
| --- | --- | --- |
| `cortex` | `repos/cortex` | Lives inside the root git worktree, so it cannot be pinned as an independent child repo without lying about topology. |
