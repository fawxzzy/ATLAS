...
from ops.atlas.supervised_execution_home_concrete_stack_command_implementation_surface import (
    ...
    CONTRACT_RECEIPT_REFS as CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CONTRACT_RECEIPT_REFS,
    QUESTION_PROMPT as CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_QUESTION_PROMPT,
...
def _base_result() -> dict[str, object]:
    ...
    return evaluate_supervised_execution_home_runtime_home_selection(
        runtime_home_selection_input
    )


class SupervisedExecutionHomeRuntimeHomeChoiceTests(unittest.TestCase):
    def _evaluate(self, bundle: dict[str, object]) -> dict[str, object]:
        payload = evaluate_supervised_execution_home_runtime_home_choice(bundle)
        self.assertEqual(
            {
                "command",
    ...
    def test_admissible_result_reopens_one_contract_local_question(self) -> None:
        ...
        self.assertEqual([], payload["runtime_home_choice_reasons"])
        self.assertEqual(
            {"question", "candidate_ref", "authoritative_receipt_refs"},
            set(payload["runtime_home_choice_question"].keys()),
        )
        self.assertEqual(
            QUESTION_PROMPT,
            payload["runtime_home_choice_question"]["question"],
        )
        self.assertEqual(
            "repos/example/.worktrees/pilot-a",
            payload["runtime_home_choice_question"]["candidate_ref"],
        )
        self.assertEqual(
            list(CONTRACT_RECEIPT_REFS),
            payload["runtime_home_choice_question"]["authoritative_receipt_refs"],
        )
        self.assertEqual(
            RUNTIME_HOME_SELECTION_STATUS_ADMISSIBLE,
            payload["runtime_home_selection_status"],
        )
        self.assertEqual(
            {
                "question": RUNTIME_HOME_SELECTION_QUESTION_PROMPT,
                "candidate_ref": "repos/example/.worktrees/pilot-a",
                "authoritative_receipt_refs": list(
                    RUNTIME_HOME_SELECTION_CONTRACT_RECEIPT_REFS
                ),
            },
            payload["runtime_home_selection_question"],
        )

    def test_non_admissible_upstream_status_fails_closed(self) -> None:
        bundle = _base_result()
        bundle["runtime_home_selection_status"] = "no_runtime_home_selection"
        bundle["runtime_home_selection_question"] = None
        bundle["runtime_home_selection_reasons"] = [
        ...
        self.assertIsNone(payload["runtime_home_choice_question"])
        self.assertEqual(
            ["runtime_home_selection_status_not_admissible"],
            payload["runtime_home_choice_reasons"],
        )
            ...
            (
                "question_card_wrong_receipts",
                {
                    "runtime_home_selection_question": {
                        ...
                        "candidate_ref": "repos/example/.worktrees/pilot-a",
                        "authoritative_receipt_refs": list(
                            RUNTIME_HOME_SELECTION_CONTRACT_RECEIPT_REFS[:-1]
                        ),
                    }
                },
                ["runtime_home_selection_question_not_explicit"],
    ...
    def test_attempted_choice_runtime_authority_or_exception_fail_closed(self) -> None:
        cases = (
            ...
            ("runtime_home", "_stack", ["runtime_home_choice_attempted"]),
            (
