...
def _base_result() -> dict[str, object]:
    ...
    return evaluate_supervised_execution_home_concrete_command_file_choice(
        concrete_command_file_choice_input
    )
...
class SupervisedExecutionHomeActualConcreteCommandFileChoiceTests(unittest.TestCase):
    def _evaluate(self, bundle: dict[str, object]) -> dict[str, object]:
        payload = evaluate_supervised_execution_home_actual_concrete_command_file_choice(
            bundle
        )
    ...
    def test_admissible_result_reopens_one_contract_local_question(self) -> None:
        ...
        self.assertEqual(
            ...
            set(payload["actual_concrete_command_file_choice_question"].keys()),
        )
        self.assertEqual(
            QUESTION_PROMPT,
            payload["actual_concrete_command_file_choice_question"]["question"],
        )
        self.assertEqual(
            "repos/example/.worktrees/pilot-a",
            payload["actual_concrete_command_file_choice_question"]["candidate_ref"],
        )
        self.assertEqual(
    ...
    def test_non_admissible_upstream_status_fails_closed(self) -> None:
        bundle = _base_result()
        bundle["concrete_command_file_choice_status"] = "no_concrete_command_file_choice"
        bundle["concrete_command_file_choice_question"] = None
        bundle["concrete_command_file_choice_reasons"] = [
            "concrete_command_file_choice_question_not_explicit"
        ]
        ...
        self.assertIsNone(payload["actual_concrete_command_file_choice_question"])
        self.assertEqual(
            ["concrete_command_file_choice_status_not_admissible"],
            payload["actual_concrete_command_file_choice_reasons"],
        )
    ...
    def test_non_explicit_question_card_or_upstream_reasons_fail_closed(self) -> None:
        cases = (
            ...
            (
                "question_card_wrong_receipts",
                {
                    "concrete_command_file_choice_question": {
                        ...
                        "candidate_ref": "repos/example/.worktrees/pilot-a",
                        "authoritative_receipt_refs": list(
                            CONCRETE_COMMAND_FILE_CHOICE_CONTRACT_RECEIPT_REFS[:-1]
                        ),
                    }
                },
                ["concrete_command_file_choice_question_not_explicit"],
            ...
            (
                ...
                ["concrete_command_file_choice_reasons_present"],
            ),
        )
    ...
    def test_attempted_choice_or_downstream_authority_fails_closed(self) -> None:
        cases = (
            (
                "actual_concrete_command_file_choice_output_present",
                {"actual_concrete_command_file_choice": "repos/_stack/ops/codex/stack.py"},
