...
from tests.test_atlas_supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_authority import (
    EXPECTED_KEYS as STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_EXPECTED_KEYS,
    _base_result as _stack_command_implementation_actual_owner_side_mutation_authority_base_result,
)

EXPECTED_KEYS = STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_EXPECTED_KEYS | {
...
def _base_result() -> dict[str, object]:
    return evaluate_supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_authority(
        _stack_command_implementation_actual_owner_side_mutation_authority_base_result()
    )


class SupervisedExecutionHomeStackCommandImplementationActualOwnerSideMutationSurfaceTests(
    unittest.TestCase
):
    def _evaluate(self, bundle: dict[str, object]) -> dict[str, object]:
        payload = evaluate_supervised_execution_home_stack_command_implementation_actual_owner_side_mutation_surface(
            bundle
        )
    ...
    def test_admissible_result_reopens_one_contract_local_question(self) -> None:
        ...
        self.assertEqual(
            {
                "question": STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_QUESTION_PROMPT,
                "candidate_ref": "repos/example/.worktrees/pilot-a",
                "authoritative_receipt_refs": list(
                    STACK_COMMAND_IMPLEMENTATION_ACTUAL_OWNER_SIDE_MUTATION_AUTHORITY_CONTRACT_RECEIPT_REFS
                ),
            },
        ...
        self.assertEqual(
            [],
            payload["stack_command_implementation_actual_owner_side_mutation_surface_reasons"],
        )
        self.assertEqual(
            {"question", "candidate_ref", "authoritative_receipt_refs"},
            set(payload["stack_command_implementation_actual_owner_side_mutation_surface_question"].keys()),
        )
        self.assertEqual(
            QUESTION_PROMPT,
    ...
    def test_non_admissible_upstream_status_fails_closed(self) -> None:
        bundle = _base_result()
        bundle["stack_command_implementation_actual_owner_side_mutation_authority_status"] = (
            "no_stack_command_implementation_actual_owner_side_mutation_authority"
        )
        bundle["stack_command_implementation_actual_owner_side_mutation_authority_question"] = None
        bundle["stack_command_implementation_actual_owner_side_mutation_authority_reasons"] = [
