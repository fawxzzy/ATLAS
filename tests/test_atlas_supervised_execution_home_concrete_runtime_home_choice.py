...
from ops.atlas.supervised_execution_home_concrete_runtime_home_choice import (
    ...
    NO_CONCRETE_RUNTIME_HOME_CHOICE,
    QUESTION_PROMPT,
...
def _base_result() -> dict[str, object]:
    ...
    return evaluate_supervised_execution_home_runtime_home_choice(runtime_home_choice_input)


class SupervisedExecutionHomeConcreteRuntimeHomeChoiceTests(unittest.TestCase):
    def _evaluate(self, bundle: dict[str, object]) -> dict[str, object]:
        payload = evaluate_supervised_execution_home_concrete_runtime_home_choice(bundle)
        self.assertEqual(
            {
                "command",
    ...
    def test_admissible_result_reopens_one_contract_local_question(self) -> None:
        ...
        self.assertEqual(
            ...
            set(payload["concrete_runtime_home_choice_question"].keys()),
        )
        self.assertEqual(
            QUESTION_PROMPT,
            payload["concrete_runtime_home_choice_question"]["question"],
        )
        self.assertEqual(
            "repos/example/.worktrees/pilot-a",
            payload["concrete_runtime_home_choice_question"]["candidate_ref"],
        )
        self.assertEqual(
            list(CONTRACT_RECEIPT_REFS),
            payload["concrete_runtime_home_choice_question"]["authoritative_receipt_refs"],
        )
        self.assertEqual(
            RUNTIME_HOME_CHOICE_STATUS_ADMISSIBLE,
    ...
    def test_non_admissible_upstream_status_fails_closed(self) -> None:
        bundle = _base_result()
        bundle["runtime_home_choice_status"] = "no_runtime_home_choice"
        bundle["runtime_home_choice_question"] = None
        bundle["runtime_home_choice_reasons"] = [
            "runtime_home_choice_question_not_explicit"
        ]
        ...
        self.assertIsNone(payload["concrete_runtime_home_choice_question"])
        self.assertEqual(
            ["runtime_home_choice_status_not_admissible"],
            payload["concrete_runtime_home_choice_reasons"],
        )
    ...
    def test_non_explicit_question_card_or_upstream_reasons_fail_closed(self) -> None:
        cases = (
            ...
            (
                "question_card_wrong_receipts",
                {
                    "runtime_home_choice_question": {
                        ...
                        "candidate_ref": "repos/example/.worktrees/pilot-a",
                        "authoritative_receipt_refs": list(
                            RUNTIME_HOME_CHOICE_CONTRACT_RECEIPT_REFS[:-1]
                        ),
                    }
                },
                ["runtime_home_choice_question_not_explicit"],
            ...
            (
                ...
                {
                    "runtime_home_choice_reasons": ["forbidden_evidence_class_used"]
                },
                ["runtime_home_choice_reasons_present"],
            ),
        )
    ...
    def test_attempted_choice_runtime_authority_or_exception_fail_closed(self) -> None:
        cases = (
            ...
            (
                "runtime_home",
                "_stack",
                ["concrete_runtime_home_choice_attempted"],
            ),
            (
