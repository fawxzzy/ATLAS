...
class SupervisedExecutionHomeConcreteCommandFileTests(unittest.TestCase):
    def _evaluate(self, bundle: dict[str, object]) -> dict[str, object]:
        payload = evaluate_supervised_execution_home_concrete_command_file(bundle)
        self.assertEqual(
            {
                "command",
    ...
    def test_admissible_result_reopens_one_contract_local_question(self) -> None:
        ...
        self.assertEqual(
            ...
            set(payload["concrete_command_file_selection_question"].keys()),
        )
        self.assertEqual(
            QUESTION_PROMPT,
            payload["concrete_command_file_selection_question"]["question"],
        )
        self.assertEqual(
            "repos/example/.worktrees/pilot-a",
            payload["concrete_command_file_selection_question"]["candidate_ref"],
        )
        self.assertEqual(
            list(CONTRACT_RECEIPT_REFS),
            payload["concrete_command_file_selection_question"]["authoritative_receipt_refs"],
        )
    ...
    def test_non_admissible_upstream_status_fails_closed(self) -> None:
        ...
        self.assertIsNone(payload["concrete_command_file_selection_question"])
        self.assertEqual(
            ["concrete_stack_command_home_selection_status_not_admissible"],
            payload["concrete_command_file_selection_reasons"],
        )
    ...
    def test_non_explicit_question_card_or_upstream_reasons_fail_closed(self) -> None:
        cases = (
            ...
            (
                ...
                {"concrete_stack_command_home_selection_reasons": ["forbidden_evidence_class_used"]},
