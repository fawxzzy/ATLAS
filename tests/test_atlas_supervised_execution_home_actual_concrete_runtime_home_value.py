...
def _base_result() -> dict[str, object]:
    ...
    return evaluate_supervised_execution_home_actual_concrete_runtime_home_value_choice(
        actual_concrete_runtime_home_value_choice_input
    )
...
class SupervisedExecutionHomeActualConcreteRuntimeHomeValueTests(unittest.TestCase):
    def _evaluate(self, bundle: dict[str, object]) -> dict[str, object]:
        payload = evaluate_supervised_execution_home_actual_concrete_runtime_home_value(
            bundle
        )
    ...
    def test_admissible_result_reopens_one_contract_local_question(self) -> None:
        ...
        self.assertEqual(
            ...
            set(payload["actual_concrete_runtime_home_value_question"].keys()),
        )
        self.assertEqual(
            QUESTION_PROMPT,
            payload["actual_concrete_runtime_home_value_question"]["question"],
        )
        self.assertEqual(
            "repos/example/.worktrees/pilot-a",
            payload["actual_concrete_runtime_home_value_question"]["candidate_ref"],
        )
        self.assertEqual(
    ...
    def test_non_admissible_upstream_status_fails_closed(self) -> None:
        bundle = _base_result()
        bundle["actual_concrete_runtime_home_value_choice_status"] = (
            "no_actual_concrete_runtime_home_value_choice"
        )
        bundle["actual_concrete_runtime_home_value_choice_question"] = None
        bundle["actual_concrete_runtime_home_value_choice_reasons"] = [
        ...
        self.assertIsNone(payload["actual_concrete_runtime_home_value_question"])
        self.assertEqual(
            ["actual_concrete_runtime_home_value_choice_status_not_admissible"],
            payload["actual_concrete_runtime_home_value_reasons"],
        )
            ...
            (
                "question_card_wrong_receipts",
                {
                    "actual_concrete_runtime_home_value_choice_question": {
                        ...
                        "candidate_ref": "repos/example/.worktrees/pilot-a",
                        "authoritative_receipt_refs": list(
                            ACTUAL_CONCRETE_RUNTIME_HOME_VALUE_CHOICE_CONTRACT_RECEIPT_REFS[:-1]
                        ),
                    }
                },
                ["actual_concrete_runtime_home_value_choice_question_not_explicit"],
