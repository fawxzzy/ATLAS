...
from ops.atlas import first_implementation_packet_ladder as ladder
...
class FirstImplementationPacketLadderTests(unittest.TestCase):
    def test_live_review_report_becomes_packet_ladder(self) -> None:
        report = _review_report(_review("first-implementation", repeat_count=7))
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            with mock.patch.object(ladder.review, "build_report", return_value=report):
                payload = ladder.build_report(root=root)

        self.assertEqual(ladder.STATUS_OK, payload["status"])
        self.assertEqual("first-implementation", payload["candidate_id"])
        self.assertEqual(7, payload["candidate_repeat_count"])
    ...
    def test_tmp_review_report_can_be_loaded(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            report_path.write_text(json.dumps(_review_report(_review("first-implementation"))), encoding="utf-8")

            payload = ladder.build_report(root=root, review_report_path="tmp/reviews.json")
    ...
    def test_review_report_path_must_be_tmp_json(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for ref in ["docs/ops/reviews.json", "tmp/reviews.txt", "../tmp/reviews.json", str(root / "tmp" / "reviews.json")]:
                payload = ladder.build_report(root=root, review_report_path=ref)
    ...
    def test_non_review_ready_candidate_blocks_ladder(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            with mock.patch.object(ladder.review, "build_report", return_value=_review_report(_review("first-implementation", status="deferred"))):
                payload = ladder.build_report(root=root)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(ladder.review, "build_report", return_value=_review_report(_review("first-implementation"))):
                for decision_ref in ["repos/app/receipt.md", "tmp/receipt.md", "runtime/receipt.md", "README.md"]:
                    payload = ladder.build_report(root=root, decision_ref=decision_ref)
            ...
            with mock.patch.object(ladder, "atlas_root", return_value=root):
                with mock.patch.object(ladder.review, "build_report", return_value=_review_report(_review("first-implementation"))):
                    ...
                    with mock.patch("sys.stdout", stdout):
                        code = ladder.main(["--json", "--output", "tmp/ladder.json"])
    ...
    def test_main_rejects_protected_output_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.object(ladder, "atlas_root", return_value=root):
                stdout = io.StringIO()
                with mock.patch("sys.stdout", stdout):
                    code = ladder.main(["--json", "--output", "docs/ops/ladder.json"])
