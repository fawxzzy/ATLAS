...
class SupervisedExecutionHomeConcreteCommandFileChoiceTests(unittest.TestCase):
    def _evaluate(self, bundle: dict[str, object]) -> dict[str, object]:
        payload = evaluate_supervised_execution_home_concrete_command_file_choice(bundle)
    ...
    def test_admissible_result_reopens_one_contract_local_question(self) -> None:
        ...
        self.assertEqual(
            CONCRETE_COMMAND_FILE_SELECTION_STATUS_ADMISSIBLE,
            payload["concrete_command_file_selection_status"],
        )
        self.assertEqual([], payload["concrete_command_file_selection_reasons"])
        self.assertEqual(
            {
                "question": CONCRETE_COMMAND_FILE_SELECTION_QUESTION_PROMPT,
                "candidate_ref": "repos/example/.worktrees/pilot-a",
                "authoritative_receipt_refs": list(
                    CONCRETE_COMMAND_FILE_SELECTION_CONTRACT_RECEIPT_REFS
                ),
            },
            payload["concrete_command_file_selection_question"],
        )
        self.assertEqual(
            CONCRETE_STACK_COMMAND_HOME_CHOICE_STATUS_ADMISSIBLE,
        ...
        self.assertEqual([], payload["concrete_command_file_choice_reasons"])
        self.assertEqual(
            {"question", "candidate_ref", "authoritative_receipt_refs"},
            set(payload["concrete_command_file_choice_question"].keys()),
        )
        self.assertEqual(
            QUESTION_PROMPT,
            payload["concrete_command_file_choice_question"]["question"],
        )
        self.assertEqual(
            "repos/example/.worktrees/pilot-a",
            payload["concrete_command_file_choice_question"]["candidate_ref"],
        )
        self.assertEqual(
    ...
    def test_non_admissible_upstream_status_fails_closed(self) -> None:
        ...
        for field, value, expected_reasons in cases:
            with self.subTest(field=field):
                ...
                self.assertIsNone(payload["concrete_command_file_choice_question"])
                self.assertEqual(
                    expected_reasons,
                    payload["concrete_command_file_choice_reasons"],
                )

    def test_non_explicit_question_card_or_upstream_reasons_fail_closed(self) -> None:
        cases = (
            (
                "command_file_question_missing_field",
                {
                    "concrete_command_file_selection_question": {
                        "question": CONCRETE_COMMAND_FILE_SELECTION_QUESTION_PROMPT,
                        "candidate_ref": "repos/example/.worktrees/pilot-a",
            ...
            (
                "command_file_question_wrong_receipts",
                {
                    "concrete_command_file_selection_question": {
                        "question": CONCRETE_COMMAND_FILE_SELECTION_QUESTION_PROMPT,
                        "candidate_ref": "repos/example/.worktrees/pilot-a",
                        "authoritative_receipt_refs": list(
                            CONCRETE_COMMAND_FILE_SELECTION_CONTRACT_RECEIPT_REFS[:-1]
                        ),
                    }
                },
                ["concrete_command_file_selection_question_not_explicit"],
            ...
            (
                "stack_home_choice_question_missing_field",
                {
                    "concrete_stack_command_home_choice_question": {
                        "question": CONCRETE_STACK_COMMAND_HOME_CHOICE_QUESTION_PROMPT,
                        "candidate_ref": "repos/example/.worktrees/pilot-a",
            ...
            (
                "stack_home_choice_question_wrong_receipts",
                {
                    "concrete_stack_command_home_choice_question": {
                        "question": CONCRETE_STACK_COMMAND_HOME_CHOICE_QUESTION_PROMPT,
                        "candidate_ref": "repos/example/.worktrees/pilot-a",
                        "authoritative_receipt_refs": list(
                            CONCRETE_STACK_COMMAND_HOME_CHOICE_CONTRACT_RECEIPT_REFS[:-1]
                        ),
                    }
                },
                ["concrete_stack_command_home_choice_question_not_explicit"],
            ...
            (
                "stack_home_choice_reasons_present",
                {
                    "concrete_stack_command_home_choice_reasons": [
                        "forbidden_evidence_class_used"
                    ]
    ...
    def test_forbidden_evidence_or_hidden_dependency_fails_closed(self) -> None:
        cases = (
            ...
            (
                "branch_dependency",
                lambda bundle: bundle.update({"branch_inventory": ["main"]}),
                ["live_repo_discovery_or_hidden_transcript_dependency"],
            ),
            (
                "worktree_dependency",
    ...
    def test_attempted_choice_or_downstream_authority_fails_closed(self) -> None:
        cases = (
            (
                "concrete_command_file_choice_output_present",
                {
                    "concrete_command_file_choice_status": (
                        CONCRETE_COMMAND_FILE_CHOICE_STATUS_ADMISSIBLE
                    )
            ...
            (
                ...
                {"concrete_stack_command_home": "_stack"},
