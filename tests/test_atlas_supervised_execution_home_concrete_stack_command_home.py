...
class SupervisedExecutionHomeConcreteStackCommandHomeTests(unittest.TestCase):
    def _evaluate(self, bundle: dict[str, object]) -> dict[str, object]:
        payload = evaluate_supervised_execution_home_concrete_stack_command_home(bundle)
        self.assertEqual(
            {
                "command",
    ...
    def test_admissible_result_reopens_one_contract_local_question(self) -> None:
        ...
        self.assertEqual(
            ...
            set(payload["concrete_stack_command_home_selection_question"].keys()),
        )
        self.assertEqual(
            QUESTION_PROMPT,
            payload["concrete_stack_command_home_selection_question"]["question"],
        )
        self.assertEqual(
            "repos/example/.worktrees/pilot-a",
            payload["concrete_stack_command_home_selection_question"]["candidate_ref"],
        )
        self.assertEqual(
            list(CONTRACT_RECEIPT_REFS),
            payload["concrete_stack_command_home_selection_question"]["authoritative_receipt_refs"],
        )

    def test_non_admissible_upstream_status_fails_closed(self) -> None:
        bundle = _base_result()
        bundle["concrete_command_home_status"] = "no_concrete_command_home"
        ...
        self.assertIsNone(payload["concrete_stack_command_home_selection_question"])
        self.assertEqual(
            ["concrete_command_home_status_not_admissible"],
            payload["concrete_stack_command_home_selection_reasons"],
        )
    ...
    def test_non_explicit_question_card_or_upstream_reasons_fail_closed(self) -> None:
        cases = (
            (
                ...
                ["concrete_command_home_question_not_explicit"],
            ),
            (
                "upstream_reasons_present",
                {"concrete_command_home_reasons": ["command_home_selection_reasons_present"]},
                ["concrete_command_home_reasons_present"],
            ),
        )
    ...
    def test_attempted_stack_choice_runtime_authority_or_exception_fail_closed(self) -> None:
        cases = (
            (
