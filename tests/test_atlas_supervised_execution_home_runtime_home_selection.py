...
class SupervisedExecutionHomeRuntimeHomeSelectionTests(unittest.TestCase):
    def _evaluate(self, bundle: dict[str, object]) -> dict[str, object]:
        payload = evaluate_supervised_execution_home_runtime_home_selection(bundle)
        self.assertEqual(
            {
                "command",
    ...
    def test_admissible_result_reopens_one_contract_local_question(self) -> None:
        ...
        self.assertEqual([], payload["runtime_home_selection_reasons"])
        self.assertEqual(
            {"question", "candidate_ref", "authoritative_receipt_refs"},
            set(payload["runtime_home_selection_question"].keys()),
        )
        self.assertEqual(
            QUESTION_PROMPT,
            payload["runtime_home_selection_question"]["question"],
        )
        self.assertEqual(
            "repos/example/.worktrees/pilot-a",
            payload["runtime_home_selection_question"]["candidate_ref"],
        )
        self.assertEqual(
            list(CONTRACT_RECEIPT_REFS),
            payload["runtime_home_selection_question"]["authoritative_receipt_refs"],
        )
    ...
    def test_non_admissible_upstream_status_fails_closed(self) -> None:
        ...
        self.assertIsNone(payload["runtime_home_selection_question"])
        self.assertEqual(
            ["concrete_stack_command_implementation_surface_selection_status_not_admissible"],
            payload["runtime_home_selection_reasons"],
        )
    ...
    def test_non_explicit_question_card_or_upstream_reasons_fail_closed(self) -> None:
        cases = (
            (
                ...
                {
                    "concrete_stack_command_implementation_surface_selection_question": {
                        "question": CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_QUESTION_PROMPT,
                        "candidate_ref": "repos/example/.worktrees/pilot-a",
                    }
                },
                [
                ...
                "question_card_wrong_receipts",
                {
                    "concrete_stack_command_implementation_surface_selection_question": {
                        ...
                        "authoritative_receipt_refs": list(
                            CONCRETE_STACK_COMMAND_IMPLEMENTATION_SURFACE_CONTRACT_RECEIPT_REFS[:-1]
                        ),
                    }
                },
                [
