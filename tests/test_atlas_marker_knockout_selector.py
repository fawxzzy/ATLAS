...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
    ...
    def test_build_campaign_selects_active_lane_from_durable_restart_truth(self) -> None:
        ...
        self.assertEqual(
            "freeze the exact root-owned contract for deciding when one preserved _stack command implementation owner-repo implementation-routing card question may or may not become one bounded downstream owner-repo mutation question without widening into actual owner-side mutation authority, _stack execution-home doctrine, Playbook export, deploy/publication, or protected-surface touch",
    ...
    def test_main_markdown_separates_current_lane_from_next_follow_on(self) -> None:
        ...
        self.assertIn(
            "current packet basis receipt: `docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-OWNER-REPO-MUTATION-CONTRACT-FREEZE-PASS-691-2026-06-26.md`",
            markdown,
        )
        self.assertIn(
        ...
        self.assertIn(
            "current packet scope: `freeze the exact root-owned contract for deciding when one preserved _stack command implementation owner-repo implementation-routing card question may or may not become one bounded downstream owner-repo mutation question without widening into actual owner-side mutation authority, _stack execution-home doctrine, Playbook export, deploy/publication, or protected-surface touch`",
            markdown,
        )
        self.assertIn("## Current Active Marker", markdown)
        self.assertIn(
            "current packet: `_stack Readiness supervised execution-home _stack command implementation owner-repo mutation contract freeze pass 691`",
            markdown,
        )
