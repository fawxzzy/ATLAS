...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
    ...
    def test_build_campaign_promotes_sandbox_after_publication_surface_claim_admission_boundary_contract(self) -> None:
        root = self._temp_root()
        ...
        self.assertIn(
            "publication-surface claim admission boundary contract",
        ...
        self.assertEqual(
            "docs/ops/SANDBOX-SIMULATION-READINESS-POST-LOCAL-ONLY-FIRST-VALIDATOR-PUBLICATION-SURFACE-CLAIM-ADMISSION-BOUNDARY-NEXT-SLICE-SELECTION-2026-06-27.md",
            payload["next_after_current_packet_basis_ref"],
        )
        self.assertEqual(
            "root-owned docs-only validator-live-unattended-execution-admission-boundary contract freeze",
            payload["next_after_current_packet_mode"],
        )
        self.assertIn(
            "whether any live unattended execution may exist above that frozen publication family at all",
            payload["next_after_current_packet_scope"],
        )
