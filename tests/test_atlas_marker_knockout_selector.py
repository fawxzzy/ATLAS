...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
    ...
    def test_build_campaign_promotes_sandbox_after_owner_surface_execution_admission_boundary_contract(self) -> None:
        root = self._temp_root()
        ...
        self.assertIn(
            "owner-surface execution admission boundary contract",
        ...
        self.assertEqual(
            "docs/ops/SANDBOX-SIMULATION-READINESS-POST-LOCAL-ONLY-FIRST-VALIDATOR-OWNER-SURFACE-EXECUTION-ADMISSION-BOUNDARY-NEXT-SLICE-SELECTION-2026-06-27.md",
            payload["next_after_current_packet_basis_ref"],
        )
        self.assertEqual(
            "root-owned docs-only validator-deploy-surface-runtime-admission-boundary contract freeze",
            payload["next_after_current_packet_mode"],
        )
        self.assertIn(
            "whether any deploy/runtime home may exist above that frozen owner-home family at all",
            payload["next_after_current_packet_scope"],
        )
