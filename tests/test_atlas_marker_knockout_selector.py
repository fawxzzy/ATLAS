...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
    ...
    def _write_packet_receipts(self, root: Path) -> None:
        current_receipt = root / "docs" / "ops" / (
            ...
            "CHOICE-SUPPORTING-LANE-ADMISSION-PASS-569-2026-06-22.md"
        )
        ...
        current_receipt.write_text(
            "\n".join(
                [
                    ...
                    "- Scope: `decide whether the owner-admitted runtime-home-choice seam honestly reopens any separate supporting seam before one runtime-home choice or one bounded first-implementation packet`",
                    "",
                ]
            ),
            encoding="utf-8",
    ...
    def test_build_campaign_selects_active_lane_from_durable_restart_truth(self) -> None:
        ...
        self.assertEqual(
            "decide whether the owner-admitted runtime-home-choice seam honestly reopens any separate supporting seam before one runtime-home choice or one bounded first-implementation packet",
    ...
    def test_main_markdown_separates_current_lane_from_next_follow_on(self) -> None:
        ...
        self.assertIn(
            "current packet basis receipt: `docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-RUNTIME-HOME-CHOICE-SUPPORTING-LANE-ADMISSION-PASS-569-2026-06-22.md`",
            markdown,
        )
        self.assertIn(
        ...
        self.assertIn(
            "current packet scope: `decide whether the owner-admitted runtime-home-choice seam honestly reopens any separate supporting seam before one runtime-home choice or one bounded first-implementation packet`",
            markdown,
        )
        self.assertIn("## Current Active Marker", markdown)
        self.assertIn(
            "current packet: `_stack Readiness supervised execution-home runtime-home choice supporting-lane admission pass 569`",
            markdown,
        )
