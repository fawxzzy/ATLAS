...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
    ...
    def _write_packet_receipts(self, root: Path) -> None:
        current_receipt = root / "docs" / "ops" / (
            ...
            "IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-AUTHORITY-FIRST-"
            "IMPLEMENTATION-WORKER-CLUSTER-RECONCILIATION-2026-06-26.md"
        )
    ...
    def test_build_campaign_selects_active_lane_from_durable_restart_truth(self) -> None:
        ...
        self.assertEqual(
            "supervised execution-home _stack command implementation actual owner-side mutation authority helper and proof worker cluster",
    ...
    def test_main_markdown_separates_current_lane_from_next_follow_on(self) -> None:
        ...
        self.assertIn(
            "current packet basis receipt: `docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-ACTUAL-OWNER-SIDE-MUTATION-AUTHORITY-FIRST-IMPLEMENTATION-WORKER-CLUSTER-RECONCILIATION-2026-06-26.md`",
            markdown,
        )
        self.assertIn(
        ...
        self.assertIn(
            "current packet scope: `supervised execution-home _stack command implementation actual owner-side mutation authority helper and proof worker cluster`",
            markdown,
        )
        self.assertIn("## Current Active Marker", markdown)
        self.assertIn(
            "current packet: `_stack Readiness supervised execution-home _stack command implementation actual owner-side mutation authority first-implementation worker cluster reconciliation`",
            markdown,
        )
