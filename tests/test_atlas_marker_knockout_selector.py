...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
        ...
        current_receipt.write_text(
            "\n".join(
                [
                    ...
                    "- Scope: `choose the strongest bounded downstream seam now that the concrete _stack command implementation-surface helper and direct proof are landed on canonical main`",
                    "",
                ]
            ),
            encoding="utf-8",
    ...
    def test_build_campaign_selects_active_lane_from_durable_restart_truth(self) -> None:
        ...
        self.assertEqual(
            "docs/ops/AI-LONG-RUN-BATCH-ORCHESTRATION-POST-CONCRETE-STACK-COMMAND-IMPLEMENTATION-SURFACE-NEXT-SLICE-SELECTION-PASS-559-2026-06-22.md",
        ...
        self.assertEqual(
            "choose the strongest bounded downstream seam now that the concrete _stack command implementation-surface helper and direct proof are landed on canonical main",
    ...
    def test_main_markdown_separates_current_lane_from_next_follow_on(self) -> None:
        ...
        self.assertIn(
            "current packet basis receipt: `docs/ops/AI-LONG-RUN-BATCH-ORCHESTRATION-POST-CONCRETE-STACK-COMMAND-IMPLEMENTATION-SURFACE-NEXT-SLICE-SELECTION-PASS-559-2026-06-22.md`",
            markdown,
        )
        self.assertIn(
        ...
        self.assertIn(
            "current packet scope: `choose the strongest bounded downstream seam now that the concrete _stack command implementation-surface helper and direct proof are landed on canonical main`",
            markdown,
        )
        self.assertIn("## Current Active Marker", markdown)
        self.assertIn(
            "current packet: `AI Long-Run Batch Orchestration post-concrete-stack-command-implementation-surface next-slice selection pass 559`",
            markdown,
        )
