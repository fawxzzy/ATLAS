...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
    ...
    def _write_packet_receipts(self, root: Path) -> None:
        current_receipt = root / "docs" / "ops" / (
            ...
            "FIRST-IMPLEMENTATION-ADMISSION-PASS-549-2026-06-22.md"
        )
        ...
        current_receipt.write_text(
            "\n".join(
                [
                    ...
                    "- Scope: `freeze the smallest fail-closed implementation slice for the root-owned concrete command-file family without choosing one concrete command file, one runtime home, _stack command implementation, worker authority, or owner-repo mutation by adjacency`",
                    "",
                ]
            ),
            encoding="utf-8",
    ...
    def test_build_campaign_selects_active_lane_from_durable_restart_truth(self) -> None:
        ...
        self.assertEqual(
            "freeze the smallest fail-closed implementation slice for the root-owned concrete command-file family without choosing one concrete command file, one runtime home, _stack command implementation, worker authority, or owner-repo mutation by adjacency",
    ...
    def test_main_markdown_separates_current_lane_from_next_follow_on(self) -> None:
        ...
        self.assertIn(
            "current packet basis receipt: `docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-CONCRETE-COMMAND-FILE-FIRST-IMPLEMENTATION-ADMISSION-PASS-549-2026-06-22.md`",
            markdown,
        )
        self.assertIn(
        ...
        self.assertIn(
            "current packet scope: `freeze the smallest fail-closed implementation slice for the root-owned concrete command-file family without choosing one concrete command file, one runtime home, _stack command implementation, worker authority, or owner-repo mutation by adjacency`",
            markdown,
        )
        self.assertIn("## Current Active Marker", markdown)
        self.assertIn(
            "current packet: `_stack Readiness supervised execution-home concrete command-file first-implementation admission pass 549`",
            markdown,
        )
