...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
        ...
        current_receipt.write_text(
            "\n".join(
                [
                    ...
                    "- Scope: `freeze the exact producing and consuming home for the _stack command implementation owner-repo mutation contract seam`",
                    "",
                ]
            ),
            encoding="utf-8",
    ...
    def test_build_campaign_selects_active_lane_from_durable_restart_truth(self) -> None:
        ...
        self.assertEqual(
            "freeze the exact producing and consuming home for the _stack command implementation owner-repo mutation contract seam",
    ...
    def test_main_markdown_separates_current_lane_from_next_follow_on(self) -> None:
        ...
        self.assertIn(
            "current packet basis receipt: `docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-STACK-COMMAND-IMPLEMENTATION-OWNER-REPO-MUTATION-OWNER-SURFACE-ADMISSION-PASS-692-2026-06-26.md`",
            markdown,
        )
        self.assertIn(
        ...
        self.assertIn(
            "current packet scope: `freeze the exact producing and consuming home for the _stack command implementation owner-repo mutation contract seam`",
            markdown,
        )
        self.assertIn("## Current Active Marker", markdown)
        self.assertIn(
            "current packet: `_stack Readiness supervised execution-home _stack command implementation owner-repo mutation owner-surface admission pass 692`",
            markdown,
        )
