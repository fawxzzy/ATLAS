...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
    ...
    def test_build_campaign_promotes_sandbox_after_unattended_runtime_proof_admission_boundary_contract(self) -> None:
        root = self._temp_root()
        ...
        self.assertIn(
            "unattended-runtime proof admission boundary contract",
        ...
        self.assertEqual(
            "docs/ops/SANDBOX-SIMULATION-READINESS-POST-LOCAL-ONLY-FIRST-VALIDATOR-UNATTENDED-RUNTIME-PROOF-ADMISSION-BOUNDARY-NEXT-SLICE-SELECTION-2026-06-27.md",
            payload["next_after_current_packet_basis_ref"],
        )
        self.assertEqual(
            "root-owned docs-only validator-publication-surface-claim-admission-boundary contract freeze",
            payload["next_after_current_packet_mode"],
        )
        self.assertIn(
            "whether any publication-safe runtime claim may exist above that frozen proof family at all",
            payload["next_after_current_packet_scope"],
        )
