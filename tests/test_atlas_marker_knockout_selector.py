...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
    ...
    def _write_packet_receipts(self, root: Path) -> None:
        current_receipt = root / "docs" / "ops" / (
            ...
            "CHOICE-IMPLEMENTATION-READINESS-CLOSEOUT-AND-WORKER-ROUTING-PASS-586-2026-06-23.md"
        )
        ...
        current_receipt.write_text(
            "\n".join(
                [
                    ...
                    "- Scope: `close the remaining root-only readiness question for the admitted actual-concrete-runtime-home-choice slice and freeze the exact bounded worker-routing result`",
                    "",
                ]
            ),
            encoding="utf-8",
    ...
    def test_build_campaign_selects_active_lane_from_durable_restart_truth(self) -> None:
        ...
        self.assertEqual(
            "docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-ACTUAL-CONCRETE-RUNTIME-HOME-CHOICE-IMPLEMENTATION-READINESS-CLOSEOUT-AND-WORKER-ROUTING-PASS-586-2026-06-23.md",
        ...
        self.assertEqual(
            "close the remaining root-only readiness question for the admitted actual-concrete-runtime-home-choice slice and freeze the exact bounded worker-routing result",
    ...
    def test_main_markdown_separates_current_lane_from_next_follow_on(self) -> None:
        ...
        self.assertIn(
            "current packet basis receipt: `docs/ops/_STACK-READINESS-SUPERVISED-EXECUTION-HOME-ACTUAL-CONCRETE-RUNTIME-HOME-CHOICE-IMPLEMENTATION-READINESS-CLOSEOUT-AND-WORKER-ROUTING-PASS-586-2026-06-23.md`",
            markdown,
        )
        self.assertIn(
        ...
        self.assertIn(
            "current packet scope: `close the remaining root-only readiness question for the admitted actual-concrete-runtime-home-choice slice and freeze the exact bounded worker-routing result`",
            markdown,
        )
        self.assertIn("## Current Active Marker", markdown)
        self.assertIn(
            "current packet: `_stack Readiness supervised execution-home actual concrete runtime-home choice implementation-readiness closeout and worker-routing pass 586`",
            markdown,
        )
