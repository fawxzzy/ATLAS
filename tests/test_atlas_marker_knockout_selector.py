...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
    ...
    def _write_packet_receipts(self, root: Path) -> None:
        current_receipt = root / "docs" / "ops" / (
            ...
            "OWNER-REPO-IMPLEMENTATION-CONTRACT-FREEZE-PASS-494-2026-06-20.md"
        )
        ...
        current_receipt.write_text(
            "\n".join(
                [
                    ...
                    "- Scope: `freeze the exact root-owned contract for deciding when one preserved selected-pilot implementation_route card may or may not become one bounded downstream owner-repo implementation packet without widening into owner-repo mutation authority, _stack execution-home doctrine, Playbook export, deploy/publication, or protected-surface touch`",
                    "",
                ]
            ),
            encoding="utf-8",
    ...
    def test_build_campaign_selects_active_lane_from_durable_restart_truth(self) -> None:
        ...
        self.assertEqual(
            "freeze the exact root-owned contract for deciding when one preserved selected-pilot implementation_route card may or may not become one bounded downstream owner-repo implementation packet without widening into owner-repo mutation authority, _stack execution-home doctrine, Playbook export, deploy/publication, or protected-surface touch",
    ...
    def test_main_markdown_separates_current_lane_from_next_follow_on(self) -> None:
        ...
        self.assertIn(
            "current packet basis receipt: `docs/ops/AI-LONG-RUN-BATCH-ORCHESTRATION-SINGLE-SUPERVISED-PILOT-SELECTED-PILOT-OWNER-REPO-IMPLEMENTATION-CONTRACT-FREEZE-PASS-494-2026-06-20.md`",
            markdown,
        )
        self.assertIn(
        ...
        self.assertIn(
            "current packet scope: `freeze the exact root-owned contract for deciding when one preserved selected-pilot implementation_route card may or may not become one bounded downstream owner-repo implementation packet without widening into owner-repo mutation authority, _stack execution-home doctrine, Playbook export, deploy/publication, or protected-surface touch`",
            markdown,
        )
        self.assertIn("## Current Active Marker", markdown)
        self.assertIn(
            "current packet: `AI Long-Run Batch Orchestration single supervised pilot selected-pilot owner-repo implementation contract freeze pass 494`",
            markdown,
        )
