...
class AtlasMarkerKnockoutSelectorTests(unittest.TestCase):
    ...
    def test_build_campaign_promotes_sandbox_after_verdict_bearing_artifact_activation_gate_contract(self) -> None:
        root = self._temp_root()
        marker_doc = MARKER_DOC.replace(
            "- Sandbox Simulation Readiness: `0%`",
            "- Sandbox Simulation Readiness: `91%`",
        )
        (root / "docs" / "atlas-book" / "02-lanes-and-markers.md").write_text(marker_doc, encoding="utf-8")
        (root / "docs" / "atlas-book" / "01-current-state.md").write_text(CURRENT_STATE_DOC, encoding="utf-8")
        self._write_packet_receipts(root)
        ...
        self.assertIn(
            "verdict-bearing artifact activation gate contract",
        ...
        self.assertEqual(
            "docs/ops/SANDBOX-SIMULATION-READINESS-POST-LOCAL-ONLY-FIRST-VALIDATOR-VERDICT-BEARING-ARTIFACT-ACTIVATION-GATE-NEXT-SLICE-SELECTION-2026-06-27.md",
            payload["next_after_current_packet_basis_ref"],
        )
        self.assertEqual(
            "root-owned docs-only validator-synchronized-artifact-writeback-boundary contract freeze",
            payload["next_after_current_packet_mode"],
        )
        self.assertIn(
            "coordinated verdict-bearing artifact truth may later write back",
            payload["next_after_current_packet_scope"],
        )
