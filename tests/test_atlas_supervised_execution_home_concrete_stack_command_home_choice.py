...
from ops.atlas.supervised_execution_home_concrete_stack_command_home import (
    ...
    evaluate_supervised_execution_home_concrete_stack_command_home,
)
from ops.atlas.supervised_execution_home_concrete_stack_command_home_choice import (
    ...
    NO_CONCRETE_STACK_COMMAND_HOME_CHOICE,
    QUESTION_PROMPT,
...
def _base_result() -> dict[str, object]:
    command_home_input = _upstream_bundle()
    command_home_result = evaluate_supervised_execution_home_command_home(command_home_input)
    concrete_command_home_result = evaluate_supervised_execution_home_concrete_command_home(
        command_home_result
    )
    return evaluate_supervised_execution_home_concrete_stack_command_home(
        concrete_command_home_result
    )
...
class SupervisedExecutionHomeConcreteStackCommandHomeChoiceTests(unittest.TestCase):
    def _evaluate(self, bundle: dict[str, object]) -> dict[str, object]:
        payload = evaluate_supervised_execution_home_concrete_stack_command_home_choice(
            bundle
        )
    ...
    def test_admissible_result_reopens_one_contract_local_question(self) -> None:
        ...
        self.assertEqual(
            COMMAND_HOME_SELECTION_STATUS_ADMISSIBLE,
            payload["command_home_selection_status"],
        )
        self.assertEqual(
            {
            ...
            payload["command_home_selection_question"],
        )
        self.assertEqual(
            CONCRETE_COMMAND_HOME_STATUS_ADMISSIBLE,
            payload["concrete_command_home_status"],
        )
        self.assertEqual(
            {
                ...
                "authoritative_receipt_refs": list(
                    CONCRETE_COMMAND_HOME_CONTRACT_RECEIPT_REFS
                ),
            },
            payload["concrete_command_home_question"],
        )
        self.assertEqual(
            CONCRETE_STACK_COMMAND_HOME_SELECTION_STATUS_ADMISSIBLE,
            payload["concrete_stack_command_home_selection_status"],
        )
        self.assertEqual([], payload["concrete_stack_command_home_selection_reasons"])
        self.assertEqual(
            {
            ...
            payload["concrete_stack_command_home_selection_question"],
        )
        self.assertEqual(
            CONCRETE_STACK_COMMAND_HOME_CHOICE_STATUS_ADMISSIBLE,
            payload["concrete_stack_command_home_choice_status"],
        )
        self.assertEqual([], payload["concrete_stack_command_home_choice_reasons"])
        self.assertEqual(
            {"question", "candidate_ref", "authoritative_receipt_refs"},
            set(payload["concrete_stack_command_home_choice_question"].keys()),
        )
        self.assertEqual(
            QUESTION_PROMPT,
            payload["concrete_stack_command_home_choice_question"]["question"],
        )
        self.assertEqual(
            "repos/example/.worktrees/pilot-a",
            payload["concrete_stack_command_home_choice_question"]["candidate_ref"],
        )
        self.assertEqual(
    ...
    def test_non_admissible_upstream_status_fails_closed(self) -> None:
        bundle = _base_result()
        bundle["concrete_stack_command_home_selection_status"] = (
            "no_concrete_stack_command_home_selection"
        )
        bundle["concrete_stack_command_home_selection_question"] = None
        bundle["concrete_stack_command_home_selection_reasons"] = [
        ...
        self.assertIsNone(payload["concrete_stack_command_home_choice_question"])
        self.assertEqual(
            ["concrete_stack_command_home_selection_status_not_admissible"],
            payload["concrete_stack_command_home_choice_reasons"],
        )
            ...
            (
                "question_card_wrong_receipts",
                {
                    "concrete_stack_command_home_selection_question": {
                        ...
                        "candidate_ref": "repos/example/.worktrees/pilot-a",
                        "authoritative_receipt_refs": list(
                            CONCRETE_STACK_COMMAND_HOME_CONTRACT_RECEIPT_REFS[:-1]
                        ),
                    }
                },
                ["concrete_stack_command_home_selection_question_not_explicit"],
    ...
    def test_attempted_choice_or_downstream_authority_fails_closed(self) -> None:
        cases = (
            (
